...
const UIThemeSettings: React.FC<UIThemeSettingsProps> = ({ userID, userRole, accessToken }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(t("uiTheme.notifications.updated"));
      ...
      NotificationsManager.fromBackend(t("uiTheme.notifications.updateFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(t("uiTheme.notifications.reset"));
      } else {
        throw new Error("Failed to reset");
      }
    } catch (error) {
      console.error("Error resetting theme settings:", error);
      NotificationsManager.fromBackend(t("uiTheme.notifications.resetFailed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full mx-auto max-w-4xl px-6 py-8">
      ...
      <Card className="shadow-xs p-6">
        <div className="space-y-6">
          <div>
            <Text className="text-sm font-medium text-gray-700 mb-2 block">{t("uiTheme.logoUrl")}</Text>
            <TextInput
              ...
              className="w-full"
            />
            <Text className="text-xs text-gray-500 mt-1">{t("uiTheme.logoHelp")}</Text>
          </div>
          <div>
            <Text className="text-sm font-medium text-gray-700 mb-2 block">{t("uiTheme.faviconUrl")}</Text>
            <TextInput
              ...
              className="w-full"
            />
            <Text className="text-xs text-gray-500 mt-1">{t("uiTheme.faviconHelp")}</Text>
          </div>
