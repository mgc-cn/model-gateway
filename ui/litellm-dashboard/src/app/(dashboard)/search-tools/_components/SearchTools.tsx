...
const SearchTools: React.FC<SearchToolsProps> = ({ accessToken, userRole, userID }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("toolManagement.search.deleted"));
      setIsDeleteModalOpen(false);
      setToolToDelete(null);
      refetch();
    } catch (error) {
      console.error("Error deleting the search tool:", error);
      NotificationsManager.error(t("toolManagement.search.deleteFailed"));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("toolManagement.search.updated"));
      ...
      NotificationsManager.error(t("toolManagement.search.updateFailed"));
    }
  };
  ...
  const renderEditForm = () => (
    <Form form={form} layout="vertical">
      <Form.Item
        ...
        label={t("toolManagement.search.toolName")}
        rules={[{ required: true, message: t("toolManagement.search.nameRequired") }]}
      >
        <Input placeholder={t("toolManagement.search.namePlaceholder")} />
      </Form.Item>

      <Form.Item
        name="search_provider"
        label={t("toolManagement.search.searchProvider")}
        rules={[{ required: true, message: t("toolManagement.search.providerRequired") }]}
      >
        <Select placeholder={t("toolManagement.search.providerPlaceholder")} loading={isLoadingProviders}>
      ...
      <Form.Item name="api_key" label={t("toolManagement.search.apiKey")} extra={t("toolManagement.search.apiKeyHelp")}>
        <Input.Password placeholder={t("toolManagement.search.apiKeyPlaceholder")} />
      </Form.Item>

      <Form.Item name="description" label={t("toolManagement.common.description")}>
        <Input.TextArea rows={3} placeholder={t("toolManagement.search.descriptionPlaceholder")} />
      </Form.Item>
    </Form>
  );

  if (!accessToken || !userRole || !userID) {
    return <div className="p-6 text-center text-gray-500">{t("toolManagement.search.missingAuth")}</div>;
  }
      ...
      <div className="w-full h-full">
        <Spin spinning={isLoadingTools} indicator={<LoadingOutlined spin />} size="large">
          <Table
            ...
            pagination={false}
            locale={{
              emptyText: t("toolManagement.search.empty"),
            }}
            size="small"
          />
        </Spin>
  ...
  return (
    <div className="w-full h-full p-6">
      <DeleteResourceModal
        ...
        title={t("toolManagement.search.deleteTitle")}
        message={t("toolManagement.search.deleteMessage")}
        resourceInformationTitle={t("toolManagement.search.info")}
        resourceInformation={
          toolToDelete
            ? [
                { label: t("toolManagement.common.name"), value: toolToDelete.search_tool_name },
                { label: "ID", value: toolToDelete.search_tool_id, code: true },
                {
                  label: t("toolManagement.common.provider"),
                  value: providerInfo?.ui_friendly_name || toolToDelete.litellm_params.search_provider,
                },
                {
                  label: t("toolManagement.common.description"),
      ...
      {/* Edit Modal */}
      <Modal
        title={t("toolManagement.search.edit")}
      ...
      <Title>{t("toolManagement.search.title")}</Title>
      <Text className="text-tremor-content mt-2">{t("toolManagement.search.description")}</Text>
      {isAdminRole(userRole) && (
        <Button className="mt-4 mb-4" onClick={() => setCreateModalVisible(true)}>
          {t("toolManagement.search.add")}
        </Button>
      )}
