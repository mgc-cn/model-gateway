        ...
        <div className="flex items-center justify-between group">
          <div className="flex items-center flex-1">
            <button
              ...
              aria-label={isExpanded ? t("caching.health.collapse") : t("caching.health.expand")}
            >
              {isExpanded ? "▼" : "▶"}
            </button>
            <div>
          ...
          <button
            ...
            aria-label={copied ? t("caching.health.copied") : t("caching.health.copy")}
          >
  ...
  try {
    if (response?.error) {
      try {
        ...
        errorDetails = {
          message: errorMessage?.message || t("caching.health.unknownError"),
        ...
        errorDetails = {
          message: String(response.error.message || t("caching.health.unknownError")),
          traceback: t("caching.health.errorParsingDetails"),
          litellm_params: {},
          ...
          <TabPanel className="p-4">
            <div>
              <div className="flex items-center mb-6">
                ...
                >
                  {t("caching.health.statusLine", {
                    label: t("caching.health.cacheStatus"),
                    status:
                      response?.status === "healthy" ? t("caching.health.healthy") : t("caching.health.unhealthy"),
                  })}
                </Text>
              </div>
              ...
              <table className="w-full border-collapse">
                <tbody>
                  ...
                  <TableClickableErrorField
                    label={t("caching.health.cacheConfiguration")}
                  ...
                  />
                  <TableClickableErrorField
                    label={t("caching.health.setCacheResponse")}
                  ...
                  {parsedLitellmParams?.type === "redis" && (
                    <>
                      ...
                      <TableClickableErrorField
                        label={t("caching.health.redisHost")}
                        value={redisDetails.redis_host || "N/A"}
                      />
                      <TableClickableErrorField
                        label={t("caching.health.redisPort")}
                      ...
                      />
                      <TableClickableErrorField
                        label={t("caching.health.namespace")}
              ...
              <pre className="whitespace-pre-wrap wrap-break-word overflow-auto max-h-[500px]">
                {(() => {
                  try {
                    ...
                    return t("caching.health.jsonFormatError", { error: (e as Error).message });
                  }
                })()}
              </pre>
            </div>
