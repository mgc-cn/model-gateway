...
export function AccessGroupEditModal({ visible, accessGroup, onCancel, onSuccess }: AccessGroupEditModalProps) {
    ...
    form
      ...
      .then((values) => {
        ...
        editMutation.mutate(
          ...
          {
            onSuccess: () => {
              MessageManager.success(t("accessGroupManagement.notifications.updated"));
              onSuccess?.();
              onCancel();
            },
          },
  ...
  return (
    <Modal
      title={t("accessGroupManagement.edit")}
      open={visible}
      onOk={handleOk}
      onCancel={onCancel}
      width={700}
      okText={t("accessGroupManagement.actions.saveChanges")}
