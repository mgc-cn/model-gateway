...
const SingleModelBreakdown: React.FC<{
  ...
  const periodLabel = t(`costTracking.results.${timePeriod === "day" ? "daily" : "monthly"}`);
    ...
    <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
      {loading && (
        <div className="flex items-center gap-2 text-gray-500 text-sm">
          ...
          <span>{t("costTracking.results.updating")}</span>
        </div>
      )}

      <div className="grid grid-cols-4 gap-4">
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.results.totalSlashRequest")}</Text>
          <Text className="text-base font-semibold text-blue-600">{formatCost(result.cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.results.inputCost")}</Text>
          <Text className="text-sm">{formatCost(result.input_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.results.outputCost")}</Text>
          <Text className="text-sm">{formatCost(result.output_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.results.marginFee")}</Text>
        ...
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-gray-200">
          <div>
            <Text className="text-xs text-gray-500 block">
              {t("costTracking.results.periodTotal", { period: periodLabel, count: formatRequests(periodRequests) })}
            </Text>
          ...
          <div>
            <Text className="text-xs text-gray-500 block">
              {t("costTracking.results.periodInput", { period: periodLabel })}
            </Text>
            <Text className="text-sm">{formatCost(periodInputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">
              {t("costTracking.results.periodOutput", { period: periodLabel })}
            </Text>
            <Text className="text-sm">{formatCost(periodOutputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">
              {t("costTracking.results.periodMargin", { period: periodLabel })}
            </Text>
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-gray-400 pt-2 border-t border-gray-200">
          {t("costTracking.results.tokenPricing")}:{" "}
          {result.input_cost_per_token && (
            <span>
              {t("costTracking.results.input")} ${formatNumberWithCommas(result.input_cost_per_token * 1_000_000, 2)}/1M
            </span>
          )}
          {result.input_cost_per_token && result.output_cost_per_token && " | "}
          {result.output_cost_per_token && (
            <span>
              {t("costTracking.results.output")} ${formatNumberWithCommas(result.output_cost_per_token * 1_000_000, 2)}
              /1M
            </span>
          )}
        </div>
...
const MultiCostResults: React.FC<MultiCostResultsProps> = ({ multiResult, timePeriod }) => {
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-gray-300 rounded-lg bg-gray-50">
        <Text className="text-gray-500">{t("costTracking.results.empty")}</Text>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <Text className="text-gray-500 block mt-2">{t("costTracking.results.calculating")}</Text>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Text className="text-base font-semibold text-gray-900">{t("costTracking.results.title")}</Text>
        ...
        {errorEntries.map((e) => (
          <div key={e.entry.id} className="text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-200">
            <span className="font-medium">{e.entry.model || t("costTracking.results.unknownModel")}: </span>
            {e.error}
          </div>
        ))}
      </div>
  ...
  const summaryColumns = [
    {
      title: t("costTracking.calculator.model"),
      dataIndex: "model",
      key: "model",
      render: (
        text: string,
        ...
        <div className="flex flex-col gap-1">
          ...
          {record.hasZeroCost && !record.error && (
            <div className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-sm">
              {t("costTracking.results.noPricing")}
            </div>
          )}
        </div>
      ),
    ...
    {
      title: t("costTracking.results.perRequest"),
    ...
    {
      title: t("costTracking.results.marginFee"),
    ...
    <div className="space-y-4">
      ...
      <Card size="small" className="bg-linear-to-r from-slate-50 to-blue-50 border-slate-200">
        <Row gutter={[16, 8]}>
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{t("costTracking.results.totalPerRequest")}</span>}
          ...
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{t("costTracking.results.totalPeriod", { period: periodLabel })}</span>}
        ...
        {hasMargin && (
          <Row gutter={[16, 8]} className="mt-3 pt-3 border-t border-slate-200">
            <Col xs={24} sm={12}>
              <div className="text-xs text-gray-500">{t("costTracking.results.marginPerRequest")}</div>
