...
const BulkEditUserModal: React.FC<BulkEditUserModalProps> = ({
  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        NotificationsManager.fromBackend(t("userManagement.bulkEdit.noChanges"));
        return;
      }
      ...
      if (hasUserUpdates) {
        if (updateAllUsers) {
          ...
          successMessages.push(t("userManagement.bulkEdit.updatedAll", { count: result.total_requested }));
        } else {
          await userBulkUpdateUserCall(accessToken, updatePayload, userIds);
          successMessages.push(t("userManagement.bulkEdit.updatedSelected", { count: userIds.length }));
        }
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (successfulTeams.length > 0) {
          ...
          successMessages.push(
            t("userManagement.bulkEdit.addedToTeams", {
              teamCount: successfulTeams.length,
              userCount: totalAdditions,
            }),
          );
        }

        if (failedTeams.length > 0) {
          MessageManager.warning(t("userManagement.bulkEdit.teamAddFailed", { count: failedTeams.length }));
        }
      }
      ...
      NotificationsManager.fromBackend(t("userManagement.bulkEdit.failed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      ...
      title={
        updateAllUsers
          ? t("userManagement.bulkEdit.titleAll")
          : t("userManagement.bulkEdit.titleSelected", { count: selectedUsers.length })
      }
      width={800}
    >
      {allowAllUsers && (
        <div className="mb-4">
          <Checkbox checked={updateAllUsers} onChange={(e) => setUpdateAllUsers(e.target.checked)}>
            <Text strong>{t("userManagement.bulkEdit.updateAll")}</Text>
          </Checkbox>
          {updateAllUsers && (
            <div style={{ marginTop: 8 }}>
              <Text type="warning" style={{ fontSize: "12px" }}>
                {t("userManagement.bulkEdit.updateAllWarning")}
              </Text>
            </div>
          )}
        </div>
      ...
      {!updateAllUsers && (
        <div className="mb-4">
          <Title level={5}>{t("userManagement.bulkEdit.selectedUsers", { count: selectedUsers.length })}</Title>
          <Table
            size="small"
            bordered
            ...
            columns={[
              {
                title: t("userManagement.fields.userId"),
                dataIndex: "user_id",
                key: "user_id",
                width: "30%",
                render: (text: string) => (
              ...
              },
              {
                ...
                render: (text: string) => (
                  <Text type="secondary" style={{ fontSize: "12px" }}>
                    {text || t("userManagement.bulkEdit.noEmail")}
                  </Text>
                ),
              },
              {
                title: t("userManagement.bulkEdit.currentRole"),
                dataIndex: "user_role",
                key: "user_role",
                width: "25%",
                render: (role: string) => (
                  <Text style={{ fontSize: "12px" }}>{getLocalizedUserRole(role, possibleUIRoles, t).label}</Text>
                ),
              },
              {
                ...
                render: (budget: number | null) => (
                  <Text style={{ fontSize: "12px" }}>
                    {budget !== null ? `$${budget}` : t("userManagement.unlimited")}
                  </Text>
                ),
              },
            ]}
      ...
      <div className="mb-4">
        <Text>
          <strong>{t("userManagement.bulkEdit.instructionsLabel")}</strong> {t("userManagement.bulkEdit.instructions")}
        </Text>
      </div>

      {/* Team Management Section */}
      <Card
        title={t("userManagement.bulkEdit.teamManagement")}
        size="small"
        className="mb-4"
      ...
      >
        <Space direction="vertical" style={{ width: "100%" }}>
          <Checkbox checked={addToTeams} onChange={(e) => setAddToTeams(e.target.checked)}>
            {t("userManagement.bulkEdit.addSelectedToTeams")}
          </Checkbox>

          {addToTeams && (
            <>
              <div>
                <Text strong>{t("userManagement.bulkEdit.selectTeams")}</Text>
                <Select
                  mode="multiple"
                  placeholder={t("userManagement.bulkEdit.selectTeamsPlaceholder")}
                  value={selectedTeams}
              ...
              <div>
                <Text strong>{t("userManagement.bulkEdit.teamBudget")}</Text>
                <InputNumber
                  placeholder={t("userManagement.bulkEdit.teamBudgetPlaceholder")}
                  value={teamBudget}
                ...
                />
                <Text type="secondary" style={{ fontSize: "12px" }}>
                  {t("userManagement.bulkEdit.teamBudgetHelp")}
                </Text>
              </div>

              <Text type="secondary" style={{ fontSize: "12px" }}>
                {t("userManagement.bulkEdit.teamRoleHelp")}
              </Text>
            </>
          )}
        </Space>
      ...
      {loading && (
        <div style={{ textAlign: "center", marginTop: "10px" }}>
          <Text>
            {updateAllUsers
              ? t("userManagement.bulkEdit.updatingAll")
              : t("userManagement.bulkEdit.updatingSelected", { count: selectedUsers.length })}
          </Text>
        </div>
      )}
    </Modal>
