...
const SCIMConfig: React.FC<SCIMConfigProps> = ({ accessToken, userID, proxySettings }) => {
  ...
  const handleCreateSCIMToken = async (values: any) => {
    if (!accessToken || !userID) {
      NotificationsManager.fromBackend(t("adminSettings.scim.loginRequired"));
      return;
    }
    ...
    try {
      ...
      NotificationsManager.success(t("adminSettings.scim.created"));
    } catch (error: any) {
      console.error("Error creating SCIM token:", error);
      NotificationsManager.fromBackend(t("adminSettings.scim.createFailed", { error: parseErrorMessage(error) }));
    } finally {
    ...
    <Grid numItems={1}>
      <Card>
        <div className="flex items-center mb-4">
          <Title>{t("adminSettings.scim.title")}</Title>
        </div>
        <Text className="text-gray-600">{t("adminSettings.scim.description")}</Text>
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <Title className="text-lg flex items-center">
                ...
                {t("adminSettings.scim.tenantUrl")}
              </Title>
            </div>
            <Text className="text-gray-600 mb-3">{t("adminSettings.scim.tenantUrlHelp")}</Text>
            <div className="flex items-center">
              <TextInput value={scimBaseUrl} disabled={true} className="grow" />
              <CopyToClipboard
                text={scimBaseUrl}
                onCopy={() => NotificationsManager.success(t("adminSettings.scim.urlCopied"))}
              >
                <TremorButton variant="primary" className="ml-2 flex items-center">
                  <CopyOutlined className="h-4 w-4 mr-1" />
                  {t("adminSettings.scim.copy")}
                </TremorButton>
              </CopyToClipboard>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <Title className="text-lg flex items-center">
                ...
                {t("adminSettings.scim.authToken")}
              </Title>
            </div>

            <Callout title={t("adminSettings.scim.usingScim")} color="blue" className="mb-4">
              {t("adminSettings.scim.usingScimHelp")}
            </Callout>
            ...
            {!tokenData ? (
              <div className="bg-gray-50 p-4 rounded-lg">
                <Form form={form} onFinish={handleCreateSCIMToken} layout="vertical">
                  <Form.Item
                    name="key_alias"
                    label={t("adminSettings.scim.tokenName")}
                    rules={[{ required: true, message: t("adminSettings.scim.tokenNameRequired") }]}
                  >
                    <TextInput placeholder={t("adminSettings.scim.tokenPlaceholder")} />
                  </Form.Item>
                  <Form.Item>
                    <TremorButton
                      variant="primary"
                    ...
                    >
                      ...
                      {t("adminSettings.scim.createToken")}
                    </TremorButton>
                  </Form.Item>
                </Form>
              </div>
              ...
              <Card className="border border-yellow-300 bg-yellow-50">
                <div className="flex items-center mb-2 text-yellow-800">
                  ...
                  <Title className="text-lg text-yellow-800">{t("adminSettings.scim.yourToken")}</Title>
                </div>
                <Text className="text-yellow-800 mb-4 font-medium">{t("adminSettings.scim.copyTokenWarning")}</Text>
                <div className="flex items-center">
                  <TextInput value={tokenData.key} className="grow mr-2 bg-white" type="password" disabled={true} />
                  <CopyToClipboard
                    text={tokenData.key}
                    onCopy={() => NotificationsManager.success(t("adminSettings.scim.tokenCopied"))}
                  >
                ...
                <TremorButton className="mt-4 flex items-center" variant="secondary" onClick={() => setTokenData(null)}>
                  ...
                  {t("adminSettings.scim.createAnother")}
                </TremorButton>
              </Card>
            )}
          </div>
