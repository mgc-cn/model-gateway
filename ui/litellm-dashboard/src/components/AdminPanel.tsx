...
const AdminPanel: React.FC<AdminPanelProps> = ({ proxySettings }) => {
  ...
  const all_ip_address_allowed = t("adminSettings.security.allIpAddresses");
  ...
  const handleShowAllowedIPs = async () => {
    try {
      if (premiumUser !== true) {
        NotificationsManager.fromBackend(t("adminSettings.notifications.premiumRequired"));
        return;
      }
      if (accessToken) {
      ...
      NotificationsManager.fromBackend(t("adminSettings.notifications.fetchIpsFailed", { error: String(error) }));
      setAllowedIPs([all_ip_address_allowed]);
    } finally {
      if (premiumUser === true) {
  ...
  const handleAddIP = async (values: { ip: string }) => {
    try {
      if (accessToken) {
        ...
        NotificationsManager.success(t("adminSettings.notifications.ipAdded"));
      }
    } catch (error) {
      console.error("Error adding IP:", error);
      NotificationsManager.fromBackend(t("adminSettings.notifications.addIpFailed", { error: String(error) }));
    } finally {
  ...
  const confirmDeleteIP = async () => {
    if (ipToDelete && accessToken) {
      try {
        ...
        NotificationsManager.success(t("adminSettings.notifications.ipDeleted"));
      } catch (error) {
        console.error("Error deleting IP:", error);
        NotificationsManager.fromBackend(t("adminSettings.notifications.deleteIpFailed", { error: String(error) }));
      } finally {
  ...
  const tabItems = [
    {
      key: "sso-settings",
      label: t("adminSettings.tabs.sso"),
    ...
    },
    {
      ...
      children: (
        <>
          <Card>
            <Title level={4}>{t("adminSettings.security.title")}</Title>
            <Alert
              message={t("adminSettings.security.ssoDeprecated")}
              description={t("adminSettings.security.ssoDeprecatedDescription")}
              type="warning"
              showIcon
            />
            <div
            ...
            >
              <div>
                <Button style={{ width: "150px" }} onClick={() => setIsAddSSOModalVisible(true)}>
                  {ssoConfigured ? t("adminSettings.sso.edit") : t("adminSettings.sso.add")}
                </Button>
              </div>
              <div>
                <Button style={{ width: "150px" }} onClick={handleShowAllowedIPs}>
                  {t("adminSettings.security.allowedIps")}
                </Button>
              </div>
              <div>
                <Button
                  ...
                  onClick={() =>
                    premiumUser === true
                      ...
                      : NotificationsManager.fromBackend(t("adminSettings.notifications.uiAccessPremium"))
                  }
                >
                  {t("adminSettings.security.uiAccessControl")}
                </Button>
              </div>
            </div>
          </Card>
          ...
          <div className="flex justify-start mb-4">
            ...
            />
            <Modal
              title={t("adminSettings.security.manageIps")}
              width={800}
              ...
              footer={[
                <Button className="mx-1" key="add" onClick={() => setIsAddIPModalVisible(true)}>
                  {t("adminSettings.security.addIp")}
                </Button>,
                <Button key="close" onClick={() => setIsAllowedIPModalVisible(false)}>
                  {t("adminSettings.actions.close")}
                </Button>,
              ]}
            >
              <Table>
                <TableHead>
                  <TableRow>
                    <TableHeaderCell>{t("adminSettings.security.ipAddress")}</TableHeaderCell>
                    <TableHeaderCell className="text-right">{t("adminSettings.actions.action")}</TableHeaderCell>
                  </TableRow>
                </TableHead>
                <TableBody>
            ...
            <Modal
              title={t("adminSettings.security.addAllowedIp")}
              open={isAddIPModalVisible}
              onCancel={() => setIsAddIPModalVisible(false)}
              footer={null}
            >
              <Form onFinish={handleAddIP}>
                <Form.Item name="ip" rules={[{ required: true, message: t("adminSettings.security.ipRequired") }]}>
                  <Input placeholder={t("adminSettings.security.ipPlaceholder")} />
                </Form.Item>
                <Form.Item>
                  <Button2 htmlType="submit">{t("adminSettings.security.addIp")}</Button2>
                </Form.Item>
              </Form>
            </Modal>

            <Modal
              title={t("adminSettings.security.confirmDelete")}
            ...
            >
              <Text>{t("adminSettings.security.deleteIpConfirmation", { ip: ipToDelete })}</Text>
            </Modal>

            {/* UI Access Control Modal */}
            <Modal
              title={t("adminSettings.security.uiAccessControlSettings")}
            ...
            >
              <UIAccessControlForm
                ...
                onSuccess={() => {
                  ...
                  NotificationsManager.success(t("adminSettings.notifications.uiAccessUpdated"));
                }}
              />
            </Modal>
          </div>
          <Callout title={t("adminSettings.security.loginWithoutSso")} color="teal">
            {t("adminSettings.security.loginWithoutSsoDescription")}{" "}
    ...
    {
      key: "scim",
      label: t("adminSettings.tabs.scim"),
    ...
    },
    {
      ...
      label: (
        <Space>
          <Text>
            {t("adminSettings.tabs.ui")} <NewBadge />
          </Text>
        </Space>
      ),
      children: <UISettings />,
    },
    {
      ...
      label: t("adminSettings.tabs.logging"),
      children: <LoggingSettings />,
    },
    {
      key: "hashicorp-vault",
      label: t("adminSettings.tabs.vault"),
      children: <HashicorpVault />,
    },
    {
      key: "plugins",
      label: t("adminSettings.tabs.plugins"),
  ...
  return (
    <div className="w-full m-2 mt-2 p-8">
      <Title level={4}>{t("adminSettings.title")}</Title>
      <Paragraph>{t("adminSettings.description")}</Paragraph>
      <Tabs items={tabItems} />
    </div>
  );
};
