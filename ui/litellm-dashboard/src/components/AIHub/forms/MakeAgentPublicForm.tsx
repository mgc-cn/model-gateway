...
const MakeAgentPublicForm: React.FC<MakeAgentPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedAgents.size === 0) {
        NotificationsManager.fromBackend(t("modelCenter.agentPublish.selectRequired"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("modelCenter.agentPublish.success", { count: agentIdsToMakePublic.length }));
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making agents public:", error);
      NotificationsManager.fromBackend(t("modelCenter.agentPublish.failed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("modelCenter.selectAgents")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("modelCenter.agentPublish.selectAll")} {agentHubData.length > 0 && `(${agentHubData.length})`}
            </Checkbox>
          </div>
        </div>

        <Text className="text-sm text-gray-600">{t("modelCenter.agentPublish.description")}</Text>

        <div className="max-h-96 overflow-y-auto border rounded-lg p-4">
          <div className="space-y-3">
            {agentHubData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("modelCenter.agentPublish.noAgents")}</Text>
              </div>
            ) : (
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {agent.skills.length > 3 && (
                            <Text className="text-xs text-gray-500">
                              {t("modelCenter.agentPublish.more", { count: agent.skills.length - 3 })}
                            </Text>
                          )}
                        </div>
                      )}
        ...
        {selectedAgents.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              {t("modelCenter.agentPublish.selected", { count: selectedAgents.size })}
            </Text>
          </div>
        )}
      </div>
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        <Title>{t("modelCenter.agentPublish.confirmTitle")}</Title>

        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <Text className="text-sm text-yellow-800">
            <strong>{t("modelCenter.agentPublish.warningLabel")}</strong> {t("modelCenter.agentPublish.warning")}
          </Text>
        </div>

        <div className="space-y-3">
          <Text className="font-medium">{t("modelCenter.agentPublish.agents")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("modelCenter.agentPublish.total", { count: selectedAgents.size })}
          </Text>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex justify-between mt-6">
        <Button onClick={currentStep === 0 ? handleClose : handlePrevious}>
          {currentStep === 0 ? t("modelCenter.agentPublish.cancel") : t("modelCenter.agentPublish.previous")}
        </Button>

        <div className="flex space-x-2">
          {currentStep === 0 && (
            <Button onClick={handleNext} disabled={selectedAgents.size === 0}>
              {t("modelCenter.agentPublish.next")}
            </Button>
          )}

          {currentStep === 1 && (
            <Button onClick={handleSubmit} loading={loading}>
              {t("modelCenter.agentPublish.submit")}
            </Button>
          )}
        </div>
      </div>
  ...
  return (
    <Modal
      title={t("modelCenter.agentPublish.modalTitle")}
      open={visible}
    ...
    >
      <Form form={form} layout="vertical">
        <Steps current={currentStep} className="mb-6">
          <Step title={t("modelCenter.agentPublish.selectStep")} />
          <Step title={t("modelCenter.agentPublish.confirmStep")} />
        </Steps>
