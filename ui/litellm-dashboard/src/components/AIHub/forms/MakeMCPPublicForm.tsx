...
const MakeMCPPublicForm: React.FC<MakeMCPPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedServers.size === 0) {
        NotificationsManager.fromBackend(t("modelCenter.mcpPublish.selectRequired"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("modelCenter.mcpPublish.success", { count: serverIdsToMakePublic.length }));
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making MCP servers public:", error);
      NotificationsManager.fromBackend(t("modelCenter.mcpPublish.failed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("modelCenter.selectMcp")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("modelCenter.mcpPublish.selectAll")} {mcpHubData.length > 0 && `(${mcpHubData.length})`}
            </Checkbox>
          </div>
        </div>

        <Text className="text-sm text-gray-600">{t("modelCenter.mcpPublish.description")}</Text>

        <div className="max-h-96 overflow-y-auto border rounded-lg p-4">
          <div className="space-y-3">
            {mcpHubData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("modelCenter.mcpPublish.noServers")}</Text>
              </div>
            ) : (
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {isPublic && (
                          <Badge color="emerald" size="sm">
                            {t("modelCenter.mcpPublish.public")}
                          </Badge>
                        )}
                        ...
                        <Badge
                          ...
                          size="sm"
                        >
                          {t(`modelCenter.mcpPublish.status.${server.status || "unknown"}`, {
                            defaultValue: server.status || t("modelCenter.mcpPublish.status.unknown"),
                          })}
                        </Badge>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {server.allowed_tools.length > 3 && (
                            <Text className="text-xs text-gray-500">
                              {t("modelCenter.mcpPublish.more", { count: server.allowed_tools.length - 3 })}
                            </Text>
                          )}
                        </div>
                      )}
        ...
        {selectedServers.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              {t("modelCenter.mcpPublish.selected", { count: selectedServers.size })}
            </Text>
          </div>
        )}
      </div>
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        <Title>{t("modelCenter.mcpPublish.confirmTitle")}</Title>

        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <Text className="text-sm text-yellow-800">
            <strong>{t("modelCenter.mcpPublish.warningLabel")}</strong> {t("modelCenter.mcpPublish.warning")}
          </Text>
        </div>

        <div className="space-y-3">
          <Text className="font-medium">{t("modelCenter.mcpPublish.servers")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("modelCenter.mcpPublish.total", { count: selectedServers.size })}
          </Text>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex justify-between mt-6">
        <Button onClick={currentStep === 0 ? handleClose : handlePrevious}>
          {currentStep === 0 ? t("modelCenter.mcpPublish.cancel") : t("modelCenter.mcpPublish.previous")}
        </Button>

        <div className="flex space-x-2">
          {currentStep === 0 && (
            <Button onClick={handleNext} disabled={selectedServers.size === 0}>
              {t("modelCenter.mcpPublish.next")}
            </Button>
          )}

          {currentStep === 1 && (
            <Button onClick={handleSubmit} loading={loading}>
              {t("modelCenter.mcpPublish.submit")}
            </Button>
          )}
        </div>
      </div>
  ...
  return (
    <Modal
      title={t("modelCenter.mcpPublish.modalTitle")}
      open={visible}
    ...
    >
      <Form form={form} layout="vertical">
        <Steps current={currentStep} className="mb-6">
          <Step title={t("modelCenter.mcpPublish.selectStep")} />
          <Step title={t("modelCenter.mcpPublish.confirmStep")} />
        </Steps>
