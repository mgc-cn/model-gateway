        ...
        return (
            ...
            rules={
              isRequired
                ? [
                    {
                      required: true,
                      message: t("loggingAndAlerts.form.fieldRequired", { field: fieldLabel }),
                    },
                  ]
                : undefined
            }
          >
            {paramType === "password" ? (
              <Input.Password
                size="large"
                placeholder={t("loggingAndAlerts.form.enterField", { field: fieldLabel })}
...
const CallbackSelector: React.FC<CallbackSelectorProps> = ({
  ...
  return (
    <FormItem
      label={t("loggingAndAlerts.form.callback")}
      name="callback"
      rules={disabled ? undefined : [{ required: true, message: t("loggingAndAlerts.form.callbackRequired") }]}
    >
      <Select
        placeholder={t("loggingAndAlerts.form.callbackPlaceholder")}
        size="large"
        className="w-full"
        showSearch
        disabled={disabled}
...
const Settings: React.FC<SettingsPageProps> = ({ accessToken, userRole, userID, premiumUser }) => {
    ...
    getCallbackConfigsCall(accessToken)
      ...
      .catch((error) => {
        NotificationsManager.fromBackend(
          t("loggingAndAlerts.notifications.configLoadFailed", { error: parseErrorMessage(error) }),
        );
      });
  }, [accessToken]);
  ...
  const alerts_to_UI_NAME: Record<string, string> = {
    llm_exceptions: t("loggingAndAlerts.alertTypes.llmExceptions"),
    llm_too_slow: t("loggingAndAlerts.alertTypes.slowResponses"),
    llm_requests_hanging: t("loggingAndAlerts.alertTypes.hangingRequests"),
    budget_alerts: t("loggingAndAlerts.alertTypes.budget"),
    db_exceptions: t("loggingAndAlerts.alertTypes.database"),
    daily_reports: t("loggingAndAlerts.alertTypes.spendReports"),
    outage_alerts: t("loggingAndAlerts.alertTypes.outage"),
    region_outage_alerts: t("loggingAndAlerts.alertTypes.regionOutage"),
  };
  ...
  const handleCallbackSubmit = async (formValues: Record<string, any>, callbackName: string, isEdit: boolean) => {
    ...
    try {
      ...
      NotificationsManager.success(
        isEdit
          ? t("loggingAndAlerts.notifications.updated")
          : t("loggingAndAlerts.notifications.added", { name: callbackName }),
      );
  ...
  const handleSaveAlerts = async () => {
    ...
    NotificationsManager.success(t("loggingAndAlerts.notifications.alertsUpdated"));
  };
  const handleSaveChanges = (callback: any) => {
    ...
    NotificationsManager.success(t("loggingAndAlerts.notifications.updated"));
  };
  ...
  const confirmDeleteCallback = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("loggingAndAlerts.notifications.deleted", { name: callbackToDelete.name }));
  ...
  return (
    <div className="w-full mx-4">
      <Grid numItems={1} className="gap-2 p-8 w-full mt-2">
        <TabGroup>
          <TabList variant="line" defaultValue="1">
            <Tab value="1">{t("loggingAndAlerts.tabs.callbacks")}</Tab>
            <Tab value="2">{t("loggingAndAlerts.tabs.cloudZero")}</Tab>
            <Tab value="3">{t("loggingAndAlerts.tabs.alertTypes")}</Tab>
            <Tab value="4">{t("loggingAndAlerts.tabs.alertSettings")}</Tab>
            <Tab value="5">{t("loggingAndAlerts.tabs.emailAlerts")}</Tab>
          </TabList>
          <TabPanels>
            <TabPanel>
              <LoggingCallbacksTable
                ...
                onTest={async (cb) => {
                  try {
                    ...
                    NotificationsManager.success(t("loggingAndAlerts.notifications.healthCheck"));
                  } catch (error) {
            ...
            <TabPanel>
              <Card>
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableHeaderCell></TableHeaderCell>
                      <TableHeaderCell>{t("loggingAndAlerts.slack.webhookUrl")}</TableHeaderCell>
                    </TableRow>
                  </TableHead>
                            ...
                            premiumUser ? (
                              ...
                              <Button className="flex items-center justify-center">
                                <a href="https://forms.gle/W3U4PZpJGFHWtHyA9" target="_blank">
                                  {t("loggingAndAlerts.enterpriseFeature")}
                                </a>
                              </Button>
                            )
                          ) : (
                ...
                <Button size="xs" className="mt-2" onClick={handleSaveAlerts}>
                  {t("loggingAndAlerts.saveChanges")}
                </Button>
                ...
                <Button
                  onClick={async () => {
                    try {
                      ...
                      NotificationsManager.success(t("loggingAndAlerts.notifications.testTriggered"));
                    } catch (error) {
                      NotificationsManager.fromBackend(parseErrorMessage(error));
                    }
                  }}
      ...
      <Modal
        title={t("loggingAndAlerts.form.addTitle")}
        ...
        >
          ...
          <div className="flex justify-end space-x-3 pt-6 mt-6 border-t border-gray-200">
            ...
            <Button2 htmlType="submit" loading={isAddingCallback} disabled={isAddingCallback}>
              {isAddingCallback ? t("loggingAndAlerts.form.adding") : t("loggingAndAlerts.addCallback")}
            </Button2>
          </div>
        </Form>
      </Modal>
      ...
      <Modal
        ...
        width={800}
        title={t("loggingAndAlerts.form.editTitle")}
        onCancel={() => {
          setShowEditCallback(false);
          setSelectedEditCallback(null);
          editForm.resetFields();
        ...
        >
          ...
          <div className="flex justify-end space-x-3 pt-6 mt-6 border-t border-gray-200">
            ...
            <Button2
              ...
              disabled={isUpdatingCallback}
            >
              {isUpdatingCallback ? t("loggingAndAlerts.form.saving") : t("loggingAndAlerts.saveChanges")}
            </Button2>
          </div>
        </Form>
      </Modal>
      ...
      <DeleteResourceModal
        ...
        title={t("loggingAndAlerts.delete.title")}
        message={t("loggingAndAlerts.delete.message")}
        resourceInformationTitle={t("loggingAndAlerts.delete.information")}
        resourceInformation={[
          { label: t("loggingAndAlerts.columns.callbackName"), value: callbackToDelete?.name },
          { label: t("loggingAndAlerts.columns.mode"), value: callbackToDelete?.mode || "success" },
        ]}
        onCancel={() => {
          setShowDeleteConfirmModal(false);
          setCallbackToDelete(null);
