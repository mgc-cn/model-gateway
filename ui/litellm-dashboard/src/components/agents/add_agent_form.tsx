...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const handleCreateAgent = async () => {
    if (!accessToken) {
      MessageManager.error(t("agentManagement.notifications.noAccessToken"));
      return;
    }
    ...
    try {
      ...
      if (!agentData) {
        MessageManager.error(t("agentManagement.notifications.buildFailed"));
      ...
      if (keyAssignOption === "create_new" && newKeyName) {
        ...
        if (!selectedExistingKey) {
          MessageManager.error(t("agentManagement.notifications.selectExistingKey"));
          setIsSubmitting(false);
          return;
        }
      ...
      MessageManager.error(
        errorMessage
          ? t("agentManagement.notifications.createFailedWithMessage", { message: errorMessage })
          : t("agentManagement.notifications.createFailed"),
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const renderEntitlementsStep = () => (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">{t("agentManagement.entitlements.description")}</p>

      <Form.Item
        label={
          <span className="text-sm font-medium text-gray-700">{t("agentManagement.entitlements.allowedModels")}</span>
        }
        name="entitlement_models"
        tooltip={t("agentManagement.entitlements.allowedModelsTooltip")}
      >
        <Select
          mode="tags"
          style={{ width: "100%" }}
          placeholder={
            loadingModels
              ? t("agentManagement.entitlements.loadingModels")
              : t("agentManagement.entitlements.selectModels")
          }
          tokenSeparators={[","]}
      ...
      <Form.Item
        label={
          <span className="text-sm font-medium text-gray-700">{t("agentManagement.entitlements.allowedAgents")}</span>
        }
        name="entitlement_agents"
        tooltip={t("agentManagement.entitlements.allowedAgentsTooltip")}
      >
        <Select
          mode="multiple"
          style={{ width: "100%" }}
          placeholder={
            loadingAgents
              ? t("agentManagement.entitlements.loadingAgents")
              : t("agentManagement.entitlements.selectAgents")
          }
      ...
      <Form.Item
        label={
          <span>
            {t("agentManagement.entitlements.allowedMcpServers")}{" "}
            <InfoCircleOutlined
              title={t("agentManagement.entitlements.allowedMcpServersTooltip")}
      ...
      >
        <MCPServerSelector
          ...
          placeholder={t("agentManagement.entitlements.mcpPlaceholder")}
        />
      </Form.Item>
  ...
  const renderObservabilityStep = () => (
    <div className="space-y-6">
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{t("agentManagement.governance.tracing")}</h4>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {t("agentManagement.governance.requireInbound")}
              </span>
              <p className="text-xs text-gray-500 mt-1">{t("agentManagement.governance.requireInboundDescription")}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {t("agentManagement.governance.requireOutbound")}
              </span>
              <p className="text-xs text-gray-500 mt-1">{t("agentManagement.governance.requireOutboundDescription")}</p>
            </div>
            <Switch
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{t("agentManagement.governance.budgetsRateLimits")}</h4>
        <div className="space-y-4">
          {!requireTraceIdOutbound && (
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
              {t("agentManagement.governance.enableTraceWarning")}
            </div>
          )}

          <div className="text-sm font-medium text-gray-700">{t("agentManagement.governance.sessionBudgets")}</div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-gray-600 block mb-1">
                {t("agentManagement.governance.maxIterations")}
              </label>
              <InputNumber
                className="w-full"
                min={1}
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t("agentManagement.governance.maxIterationsHelp")}</p>
            </div>
            <div>
              <label className="text-sm text-gray-600 block mb-1">
                {t("agentManagement.governance.maxBudgetPerSession")}
              </label>
              <InputNumber
                className="w-full"
                min={0.01}
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t("agentManagement.governance.maxBudgetHelp")}</p>
            </div>
          </div>

          <Divider className="my-2" />

          <div className="text-sm font-medium text-gray-700">{t("agentManagement.governance.agentRateLimits")}</div>
          <p className="text-xs text-gray-500">{t("agentManagement.governance.agentRateLimitsDescription")}</p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={t("agentManagement.detail.tpmLimit")} name="tpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder="e.g. 100000" disabled={!requireTraceIdOutbound} />
            </Form.Item>
            <Form.Item label={t("agentManagement.detail.rpmLimit")} name="rpm_limit" className="mb-0">
          ...
          <div className="text-sm font-medium text-gray-700 mt-4">
            {t("agentManagement.governance.perSessionRateLimits")}
          </div>
          <p className="text-xs text-gray-500">{t("agentManagement.governance.perSessionRateLimitsDescription")}</p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={t("agentManagement.detail.sessionTpmLimit")} name="session_tpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder="e.g. 10000" disabled={!requireTraceIdOutbound} />
            </Form.Item>
            <Form.Item label={t("agentManagement.detail.sessionRpmLimit")} name="session_rpm_limit" className="mb-0">
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{t("agentManagement.governance.guardrails")}</h4>
        <p className="text-xs text-gray-500 mb-3">{t("agentManagement.governance.guardrailsDescription")}</p>
  ...
  const renderConfigureStep = () => (
    <>
      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{t("agentManagement.form.agentType")}</span>}
        required
        tooltip={t("agentManagement.form.agentTypeTooltip")}
      >
        <Select
          value={agentType}
            ...
            <>
              ...
              <div className="px-2 py-1">
                <div className="text-xs text-gray-400 font-medium mb-1 uppercase tracking-wide px-2">
                  {t("agentManagement.modal.custom.notListed")}
                </div>
                <div
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-amber-700">{t("agentManagement.modal.custom.title")}</span>
                      <Tag color="orange" style={{ fontSize: 10, padding: "0 4px" }}>
                        {t("agentManagement.modal.custom.tag")}
                      </Tag>
                    </div>
                    <div className="text-xs text-amber-600">{t("agentManagement.modal.custom.description")}</div>
                  </div>
                </div>
              </div>
            </>
        ...
        {agentType === CUSTOM_AGENT_TYPE ? (
          <div className="space-y-4">
            <Form.Item
              ...
              name="agent_name"
              rules={[{ required: true, message: t("agentManagement.form.agentNameRequiredShort") }]}
            >
              <Input placeholder={t("agentManagement.modal.custom.namePlaceholder")} />
            </Form.Item>
            <Form.Item label={t("agentManagement.form.description")} name="description">
              <Input.TextArea placeholder={t("agentManagement.form.descriptionPlaceholder")} rows={3} />
            </Form.Item>
          </div>
        ) : agentType === "a2a" ? (
            ...
            {selectedAgentTypeInfo.credential_fields.length > 0 && (
              <div className="mt-4 p-4 border border-gray-200 rounded-lg">
                <h4 className="text-sm font-medium text-gray-700 mb-3">
                  {t("agentManagement.form.dynamicSettings", { type: selectedAgentTypeInfo.agent_type_display_name })}
                </h4>
                {selectedAgentTypeInfo.credential_fields.map((field) => (
                  <Form.Item
                    ...
                    rules={
                      field.required
                        ? [{ required: true, message: t("agentManagement.form.requiredField", { label: field.label }) }]
                        : undefined
                    }
  ...
  const renderAssignKeyStep = () => {
    const agentName = form.getFieldValue("agent_name") || t("agentManagement.assignKey.defaultAgentName");
    return (
      <div>
        {/* Agent name chip */}
        ...
        <Form.Item
          label={<span className="text-sm font-medium text-gray-700">{t("agentManagement.assignKey.assignTeam")}</span>}
          name="team_id"
          tooltip={t("agentManagement.assignKey.assignTeamTooltip")}
        >
          <TeamDropdown />
        </Form.Item>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-900">{t("agentManagement.assignKey.createNew")}</span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">{t("agentManagement.assignKey.createNewDescription")}</p>
                  {keyAssignOption === "create_new" && (
                    <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <div>
                        <label className="text-sm text-gray-600 block mb-1">
                          {t("agentManagement.assignKey.keyName")}
                        </label>
                        <Input
                          value={newKeyName}
                          onChange={(e) => setNewKeyName(e.target.value)}
                          placeholder={t("agentManagement.assignKey.keyNamePlaceholder")}
                        />
                      </div>
                    </div>
                  )}
              ...
              <Tag color="green">{t("agentManagement.assignKey.recommended")}</Tag>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">{t("agentManagement.assignKey.existing")}</span>
                </div>
                <p className="text-sm text-gray-500 mt-1">{t("agentManagement.assignKey.existingDescription")}</p>
                {keyAssignOption === "existing_key" && (
                  <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                    <Select
                      showSearch
                      style={{ width: "100%" }}
                      placeholder={t("agentManagement.assignKey.searchKey")}
                      loading={loadingKeys}
        ...
        <div className="text-center mt-4">
          ...
          >
            {t("agentManagement.assignKey.skip")}
          </button>
        </div>
      </div>
    );
  ...
  const renderReadyStep = () => (
    <div className="text-center py-6">
      ...
      <h3 className="text-xl font-semibold text-gray-900 mb-2">{t("agentManagement.ready.title")}</h3>
      ...
      {assignedKeyAlias && (
        <p className="text-sm text-gray-600 mt-2">
          {t("agentManagement.ready.keyAssigned", { alias: assignedKeyAlias })}
        </p>
      )}
      {!createdKeyValue && !assignedKeyAlias && keyAssignOption === "skip" && (
        <p className="text-sm text-gray-500 mt-2">{t("agentManagement.ready.noKeyAssigned")}</p>
      )}
    </div>
  );
        ...
        <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
          {selectedLogo && currentStep < 1 && (
            <img
              ...
              alt={t("agentManagement.title")}
              className="w-6 h-6 object-contain"
            />
          )}
          <h2 className="text-xl font-semibold text-gray-900">{t("agentManagement.modal.title")}</h2>
        </div>
      }
      open={visible}
    ...
    >
      <div className="mt-4">
        ...
        <Steps current={currentStep} size="small" className="mb-8">
          <Step title={t("agentManagement.modal.steps.configure")} />
          <Step title={t("agentManagement.modal.steps.entitlements")} />
          <Step title={t("agentManagement.modal.steps.governance")} />
          <Step title={t("agentManagement.modal.steps.agentManagement")} />
          <Step title={t("agentManagement.modal.steps.ready")} />
        </Steps>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-gray-100 mt-6">
          <div>
            {currentStep > 0 && currentStep < 4 && (
              ...
              >
                {t("agentManagement.modal.actions.back")}
              </button>
            )}
          </div>
          <div className="flex gap-3">
            ...
            {currentStep === 0 && (
              <Button variant="primary" onClick={handleNext}>
                {t("agentManagement.modal.actions.next")}
              </Button>
            )}
            {currentStep === 1 && (
              <Button variant="primary" onClick={handleNext}>
            ...
            {currentStep === 3 && (
              <Button variant="primary" loading={isSubmitting} onClick={handleCreateAgent}>
                {isSubmitting ? t("agentManagement.modal.actions.creating") : t("agentManagement.modal.actions.create")}
              </Button>
            )}
            {currentStep === 4 && (
              <Button variant="primary" onClick={handleClose}>
                {t("agentManagement.modal.actions.done")}
              </Button>
            )}
          </div>
        </div>
