...
const GuardrailsPanel: React.FC<GuardrailsPanelProps> = ({ accessToken, userRole }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("guardrailManagement.delete.success", { name: guardrailToDelete.guardrail_name }));
      await fetchGuardrails();
    } catch (error) {
      console.error("Error deleting guardrail:", error);
      NotificationsManager.fromBackend(t("guardrailManagement.delete.failed"));
    } finally {
      setIsDeleting(false);
        ...
        items={[
          ...(isAdmin
            ? [
                {
                  key: "garden",
                  label: t("guardrailManagement.tabs.garden"),
                  children: <GuardrailGarden accessToken={accessToken} onGuardrailCreated={handleSuccess} />,
                },
                {
                  key: "guardrails",
                  label: t("guardrailManagement.tabs.guardrails"),
                  children: (
                    <>
                      <div className="flex justify-between items-center mb-4">
                            ...
                            items: [
                              {
                                key: "provider",
                                icon: <PlusOutlined />,
                                label: t("guardrailManagement.actions.addProvider"),
                                onClick: handleAddGuardrail,
                              },
                              {
                                key: "custom_code",
                                icon: <CodeOutlined />,
                                label: t("guardrailManagement.actions.customCode"),
                        ...
                        >
                          <Button disabled={!accessToken}>
                            {t("guardrailManagement.actions.add")} <DownOutlined className="ml-2" />
                          </Button>
                        </Dropdown>
                      </div>
                      ...
                      <DeleteResourceModal
                        ...
                        title={t("guardrailManagement.delete.title")}
                        message={t("guardrailManagement.delete.message", { name: guardrailToDelete?.guardrail_name })}
                        resourceInformationTitle={t("guardrailManagement.delete.info")}
                        resourceInformation={[
                          { label: t("guardrailManagement.fields.name"), value: guardrailToDelete?.guardrail_name },
                          { label: "ID", value: guardrailToDelete?.guardrail_id, code: true },
                          { label: t("guardrailManagement.fields.provider"), value: providerDisplayName },
                          {
                            label: t("guardrailManagement.fields.mode"),
                            value: guardrailToDelete?.litellm_params.mode,
                          },
                          {
                            label: t("guardrailManagement.fields.defaultOn"),
                            value: guardrailToDelete?.litellm_params.default_on
                              ? t("guardrailManagement.values.yes")
                              : t("guardrailManagement.values.no"),
                          },
                        ]}
                ...
                {
                  key: "playground",
                  label: t("guardrailManagement.tabs.playground"),
                  disabled: !accessToken,
                  children: (
          ...
          {
            key: "submitted",
            label: t("guardrailManagement.tabs.submitted"),
