...
const ModelSettingsModal: React.FC<ModelSettingsModalProps> = ({ isVisible, onCancel, onSuccess }) => {
  ...
  const handleFormSubmit = async (formValues: StoreModelInDBParams) => {
    try {
      await mutateAsync(formValues, {
        onSuccess: () => {
          NotificationsManager.success(t("modelsAndEndpoints.modelSettings.notifications.updated"));
          refetch();
          onSuccess?.();
        },
        onError: (error) => {
          NotificationsManager.fromBackend(
            t("modelsAndEndpoints.modelSettings.notifications.failed", { error: parseErrorMessage(error) }),
          );
        },
      });
    } catch (error) {
  ...
  return (
    <Modal
      title={<Typography.Title level={5}>{t("modelsAndEndpoints.modelSettings.title")}</Typography.Title>}
      open={isVisible}
      footer={
        <Space>
          ...
          <Button type="primary" loading={isPending} disabled={isLoadingConfig} onClick={() => form.submit()}>
            {isPending ? t("modelsAndEndpoints.modelSettings.saving") : t("modelsAndEndpoints.modelSettings.save")}
          </Button>
        </Space>
      }
    ...
    >
      ...
      >
        <Form.Item
          label={t("modelsAndEndpoints.modelSettings.storeInDb")}
          name="store_model_in_db"
          tooltip={
            proxyConfigData?.find((f) => f.field_name === "store_model_in_db")?.field_description ||
            t("modelsAndEndpoints.modelSettings.storeInDbTooltip")
          }
