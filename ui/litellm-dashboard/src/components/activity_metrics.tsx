  ...
  return (
    <div className="space-y-2">
      ...
      <Grid numItems={4} className="gap-4">
        <Card>
          <Text>{t("observability.usage.totalRequests")}</Text>
          <Title>{metrics.total_requests.toLocaleString()}</Title>
        </Card>
        <Card>
          <Text>{t("observability.usage.activity.totalSuccessfulRequests")}</Text>
          <Title>{metrics.total_successful_requests.toLocaleString()}</Title>
        </Card>
        <Card>
          <Text>{t("observability.usage.totalTokens")}</Text>
          <Title>{metrics.total_tokens.toLocaleString()}</Title>
          <Text>
            {t("observability.usage.activity.averagePerSuccessfulRequest", {
              value: Math.round(metrics.total_tokens / metrics.total_successful_requests),
            })}
          </Text>
        </Card>
        <Card>
          <Text>{t("observability.usage.activity.totalSpend")}</Text>
          <Title>${formatNumberWithCommas(metrics.total_spend, 2)}</Title>
          <Text>
            {t("observability.usage.activity.spendPerSuccessfulRequest", {
      ...
      {metrics.top_api_keys && metrics.top_api_keys.length > 0 && (
        <Card className="mt-4">
          <Title>{t("observability.usage.activity.topKeysBySpend")}</Title>
          <div className="mt-3">
              ...
              {metrics.top_api_keys.map((keyData, index) => (
                <div key={keyData.api_key} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    {keyData.team_id && (
                      <Text className="text-xs text-gray-500">
                        {t("observability.usage.activity.team", { team: keyData.team_id })}
                      </Text>
                    )}
                  </div>
                  <div className="text-right">
                    ...
                    <Text className="text-xs text-gray-500">
                      {t("observability.usage.activity.requestsAndTokens", {
      ...
      <Card className="mt-4">
        <div className="flex justify-between items-center">
          <Title>{t("observability.usage.activity.spendPerDay")}</Title>
      ...
      <Grid numItems={2} className="gap-4 mt-4">
        ...
        <Card>
          <div className="flex justify-between items-center">
            <Title>{t("observability.usage.activity.requestsPerDay")}</Title>
        ...
        <Card>
          <div className="flex justify-between items-center">
            <Title>{t("observability.usage.activity.successVsFailed")}</Title>
            <CustomLegend
        ...
        {!hidePromptCachingMetrics && (
          <Card>
            <div className="flex justify-between items-center">
              <Title>{t("observability.usage.activity.promptCaching")}</Title>
              <CustomLegend
            ...
            <div className="mb-2">
              <Text>
                {t("observability.usage.activity.cacheRead", {
                  count: metrics.total_cache_read_input_tokens?.toLocaleString() || 0,
                })}
              </Text>
              <Text>
                {t("observability.usage.activity.cacheCreation", {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border rounded-lg p-4">
        <Title>{t("observability.usage.activity.overallUsage")}</Title>
        ...
        <Grid numItems={2} className="gap-4">
          <Card>
            <div className="flex justify-between items-center">
              <Title>{t("observability.usage.activity.totalTokensOverTime")}</Title>
              <CustomLegend
                categories={["metrics.prompt_tokens", "metrics.completion_tokens", "metrics.total_tokens"]}
                colors={["blue", "cyan", "indigo"]}
              />
          ...
          <Card>
            <div className="flex justify-between items-center">
              <Title>{t("observability.usage.activity.totalRequestsOverTime")}</Title>
              <CustomLegend
                categories={["metrics.successful_requests", "metrics.failed_requests"]}
                colors={["emerald", "red"]}
              />
      ...
      <Collapse defaultActiveKey={modelNames[0]}>
        {modelNames.map((modelName) => (
          <Collapse.Panel
            ...
            header={
              <div className="flex justify-between items-center w-full">
                <Title>{modelMetrics[modelName].label || t("observability.usage.activity.unknownItem")}</Title>
                <div className="flex space-x-4 text-sm text-gray-500">
                  <span>${formatNumberWithCommas(modelMetrics[modelName].total_spend, 2)}</span>
                  <span>
                    {t("observability.usage.activity.requestCount", {
          ...
          >
            <ModelSection
              modelName={modelName || t("observability.usage.activity.unknownModel")}
