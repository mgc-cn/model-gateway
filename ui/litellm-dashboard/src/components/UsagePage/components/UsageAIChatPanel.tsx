...
const UsageAIChatPanel: React.FC<UsageAIChatPanelProps> = ({ open, onClose, accessToken }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMsg = error?.message || t("observability.usage.ai.failed");
      setMessages((prev) => [...prev, { role: "assistant", content: `Error: ${errorMsg}` }]);
      setStreamingContent("");
    } finally {
      setIsLoading(false);
      ...
      <div className="px-5 pt-5 pb-3 border-b border-gray-100 shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-gray-900">{t("observability.usage.ai.title")}</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <p className="text-xs text-gray-500">{t("observability.usage.ai.description")}</p>
      </div>

      {/* Model selector */}
      <div className="px-5 py-3 border-b border-gray-100 shrink-0">
        <Select
          placeholder={t("observability.usage.ai.modelPlaceholder")}
          value={selectedModel}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
        {messages.length === 0 && !streamingContent && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            ...
            <p className="text-sm font-medium">{t("observability.usage.ai.prompt")}</p>
        ...
        {isLoading && !streamingContent && (
          <div className="flex items-center gap-2 px-3 py-2 text-xs text-gray-500">
            <Spin size="small" />
            <span className="italic">{statusMessage || t("observability.usage.ai.thinking")}</span>
          </div>
        )}
      ...
      <div className="px-4 py-3 border-t border-gray-200 bg-white shrink-0">
        <div className="flex gap-2">
          <TextArea
            ...
            placeholder={t("observability.usage.ai.inputPlaceholder")}
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <span className="text-xs text-gray-400">{t("observability.usage.ai.sendHint")}</span>
        </div>
      </div>
    </div>
  );
