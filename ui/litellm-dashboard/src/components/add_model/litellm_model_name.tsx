...
const LiteLLMModelNameField: React.FC<LiteLLMModelNameFieldProps> = ({
  ...
  return (
    <>
      <Form.Item
        label={t("modelsAndEndpoints.addModel.model.label")}
        tooltip={t("modelsAndEndpoints.addModel.model.tooltip")}
        className="mb-0"
      >
        <Form.Item
          name="model"
          rules={[
            {
              ...
              message:
                selectedProvider === Providers.Azure
                  ? t("modelsAndEndpoints.addModel.model.azureRequired")
                  : t("modelsAndEndpoints.addModel.model.modelRequired"),
            },
          ]}
          noStyle
        >
          ...
          selectedProvider === Providers.Ollama ? (
            ...
            <AntSelect
              ...
              allowClear
              showSearch
              placeholder={t("modelsAndEndpoints.addModel.model.select")}
              onChange={handleModelChange}
              optionFilterProp="children"
              filterOption={(input, option) => (option?.label ?? "").toLowerCase().includes(input.toLowerCase())}
              options={[
                {
                  label: t("modelsAndEndpoints.addModel.model.customOption"),
                  value: "custom",
                },
                {
                  label: t("modelsAndEndpoints.addModel.model.wildcardOption", { provider: selectedProvider }),
                  value: "all-wildcard",
                },
            ...
            return (
              modelArray.includes("custom") && (
                <Form.Item
                  ...
                  rules={[{ required: true, message: t("modelsAndEndpoints.addModel.model.customRequired") }]}
                  className="mt-2"
                >
                  <TextInput
                    placeholder={
                      selectedProvider === Providers.Azure
                        ? t("modelsAndEndpoints.addModel.model.azurePlaceholder")
                        : t("modelsAndEndpoints.addModel.model.customPlaceholder")
                    }
      ...
      <Row>
        ...
        <Col span={14}>
          <Text className="mb-3 mt-1">
            {selectedProvider === Providers.Azure
              ? t("modelsAndEndpoints.addModel.model.azureHelp")
              : t("modelsAndEndpoints.addModel.model.defaultHelp")}
          </Text>
        </Col>
      </Row>
    </>
