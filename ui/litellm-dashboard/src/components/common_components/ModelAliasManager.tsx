...
const ModelAliasManager: React.FC<ModelAliasManagerProps> = ({
  ...
  const handleAddAlias = () => {
    if (!newAlias.aliasName || !newAlias.targetModel) {
      NotificationsManager.fromBackend(t("virtualKeys.create.aliasManager.required"));
      return;
    }

    // Check for duplicate alias names
    if (aliases.some((alias) => alias.aliasName === newAlias.aliasName)) {
      NotificationsManager.fromBackend(t("virtualKeys.create.aliasManager.duplicate"));
      return;
    }
    ...
    NotificationsManager.success(t("virtualKeys.create.aliasManager.added"));
  };
  ...
  const handleUpdateAlias = () => {
    ...
    NotificationsManager.success(t("virtualKeys.create.aliasManager.updated"));
  };
  ...
  const deleteAlias = (aliasId: string) => {
    ...
    NotificationsManager.success(t("virtualKeys.create.aliasManager.deleted"));
  };
  ...
  return (
    <div className="mt-4">
      <div className="mb-6">
        <Text className="text-sm font-medium text-gray-700 mb-2">{t("virtualKeys.create.aliasManager.addTitle")}</Text>
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t("virtualKeys.create.aliasManager.aliasName")}</label>
            <input
              type="text"
              ...
              placeholder={t("virtualKeys.create.aliasManager.aliasPlaceholder")}
              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
            />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">
              {t("virtualKeys.create.aliasManager.targetModel")}
            </label>
            <ModelSelector
              accessToken={accessToken}
              value={newAlias.targetModel}
              placeholder={t("virtualKeys.create.aliasManager.targetPlaceholder")}
              onChange={(value) =>
                setNewAlias({
                  ...newAlias,
                  targetModel: value,
          ...
          <div className="flex items-end">
            ...
            >
              ...
              {t("virtualKeys.create.aliasManager.add")}
            </button>
          </div>
        </div>
      </div>

      <Text className="text-sm font-medium text-gray-700 mb-2">{t("virtualKeys.create.aliasManager.manage")}</Text>
      <div className="rounded-lg custom-border relative mb-6">
        <div className="overflow-x-auto">
          <Table className="[&_td]:py-0.5 [&_th]:py-1">
            <TableHead>
              <TableRow>
                <TableHeaderCell className="py-1 h-8">{t("virtualKeys.create.aliasManager.aliasName")}</TableHeaderCell>
                <TableHeaderCell className="py-1 h-8">
                  {t("virtualKeys.create.aliasManager.targetModel")}
                </TableHeaderCell>
                <TableHeaderCell className="py-1 h-8">{t("virtualKeys.create.aliasManager.actions")}</TableHeaderCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {aliases.map((alias) => (
                      ...
                      <TableCell className="py-0.5 whitespace-nowrap">
                        <div className="flex space-x-2">
                          <button
                            ...
                            title={t("virtualKeys.create.aliasManager.edit")}
                            className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-sm hover:bg-blue-100"
                          >
                          ...
                          <button
                            ...
                            title={t("virtualKeys.create.aliasManager.delete")}
              ...
              {aliases.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="py-0.5 text-sm text-gray-500 text-center">
                    {t("virtualKeys.create.aliasManager.empty")}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      {showExampleConfig && (
        <Card>
          <Title className="mb-4">{t("virtualKeys.create.aliasManager.configExample")}</Title>
          <Text className="text-gray-600 mb-4">{t("virtualKeys.create.aliasManager.configDescription")}</Text>
            ...
            <div className="text-gray-700">
              ...
              {Object.keys(aliasObject).length === 0 ? (
                <span className="text-gray-500">
                  <br />
                  &nbsp;&nbsp;{t("virtualKeys.create.aliasManager.noAliases")}
                </span>
              ) : (
