...
const GuardrailTestPlayground: React.FC<GuardrailTestPlaygroundProps> = ({
  ...
  const handleTestGuardrails = async (text: string) => {
    ...
    if (results.length > 0) {
      NotificationsManager.success(t("guardrailManagement.playground.success", { count: results.length }));
    }
    if (errors.length > 0) {
      NotificationsManager.fromBackend(t("guardrailManagement.playground.failed", { count: errors.length }));
    }
  };
        ...
        <div className="flex h-full">
          ...
          <div className="w-1/4 border-r border-gray-200 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-gray-200">
              <div className="mb-3">
                <h3 className="text-lg font-semibold mb-3">{t("guardrailManagement.playground.guardrails")}</h3>
                <Input
                  prefix={<SearchOutlined />}
                  placeholder={t("guardrailManagement.playground.search")}
                  value={searchQuery}
              ...
              {isLoading ? (
                ...
                <div className="p-4">
                  <Empty
                    description={
                      searchQuery
                        ? t("guardrailManagement.playground.noMatches")
                        : t("guardrailManagement.playground.noneAvailable")
                    }
                  />
                </div>
              ) : (
                      ...
                      <List.Item.Meta
                        ...
                        description={
                          <div className="text-xs space-y-1 mt-1">
                            <div>
                              <span className="font-medium">{t("guardrailManagement.playground.type")} </span>
                              <span className="text-gray-600">{guardrail.litellm_params.guardrail}</span>
                            </div>
                            <div>
                              <span className="font-medium">{t("guardrailManagement.playground.mode")} </span>
            ...
            <div className="p-3 border-t border-gray-200 bg-gray-50">
              <Typography.Text className="text-xs text-gray-600">
                {t("guardrailManagement.playground.selected", {
          ...
          <div className="w-3/4 flex flex-col bg-white">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <Typography.Title level={2} className="text-xl font-semibold mb-0">
                {t("guardrailManagement.playground.title")}
              </Typography.Title>
            </div>
              ...
              {selectedGuardrails.size === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400">
                  ...
                  <Typography.Paragraph className="text-lg font-medium text-gray-600 mb-2">
                    {t("guardrailManagement.playground.selectTitle")}
                  </Typography.Paragraph>
                  <Typography.Paragraph className="text-center text-gray-500 max-w-md">
                    {t("guardrailManagement.playground.selectDescription")}
                  </Typography.Paragraph>
                </div>
              ) : (
