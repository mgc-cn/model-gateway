...
const GuardrailTable: React.FC<GuardrailTableProps> = ({
  ...
  const columns: ColumnDef<Guardrail>[] = [
    {
      header: t("guardrailManagement.fields.id"),
    ...
    {
      header: t("guardrailManagement.fields.name"),
    ...
    {
      header: t("guardrailManagement.fields.provider"),
    ...
    {
      header: t("guardrailManagement.fields.mode"),
    ...
    {
      header: t("guardrailManagement.fields.defaultOn"),
        ...
        return (
          ...
          >
            {guardrail.litellm_params?.default_on
              ? t("guardrailManagement.values.defaultOn")
              : t("guardrailManagement.values.defaultOff")}
          </Badge>
        );
      },
    },
    {
      header: t("guardrailManagement.fields.createdAt"),
    ...
    {
      header: t("guardrailManagement.fields.updatedAt"),
    ...
    {
      id: "actions",
      header: t("guardrailManagement.fields.actions"),
      cell: ({ row }) => {
        const guardrail = row.original;
        ...
        return (
          <div className="flex space-x-2">
            {isConfigGuardrail ? (
              <Tooltip title={t("guardrailManagement.table.configDeleteHelp")}>
                <Icon
                  data-testid="config-delete-icon"
                  icon={TrashIcon}
                  size="sm"
                  className="cursor-not-allowed text-gray-400"
                  title={t("guardrailManagement.table.configDeleteHelp")}
                  aria-label={t("guardrailManagement.table.configDeleteLabel")}
                />
              </Tooltip>
            ) : (
              <Tooltip title={t("guardrailManagement.actions.delete")}>
                <Icon
                  icon={TrashIcon}
                  size="sm"
                  onClick={() =>
                    ...
                    onDeleteClick(
                      guardrail.guardrail_id,
                      guardrail.guardrail_name || t("guardrailManagement.values.unnamed"),
                    )
                  }
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={columns.length} className="h-8 text-center">
                  <div className="text-center text-gray-500">
                    <p>{t("guardrailManagement.table.loading")}</p>
                  </div>
                </TableCell>
              </TableRow>
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-8 text-center">
                  <div className="text-center text-gray-500">
                    <p>{t("guardrailManagement.table.empty")}</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
