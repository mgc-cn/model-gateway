    ...
    <div className="p-4 max-w-full">
      <div className="mb-6">
        <Button icon={ArrowLeftIcon} variant="light" className="mb-4" onClick={onBack}>
          {t("mcpManagement.detail.back")}
        </Button>
        <div className="flex items-center gap-2">
          <Title className="text-2xl">
            {mcpServer.server_name || mcpServer.alias || t("mcpManagement.values.unnamed")}
          </Title>
          <AntdButton
            type="text"
            size="small"
      ...
      <TabGroup index={selectedTabIndex} onIndexChange={setSelectedTabIndex}>
        <TabList className="mb-4">
          {[
            <Tab key="overview">{t("mcpManagement.tabs.overview")}</Tab>,
            <Tab key="tools">{t("mcpManagement.tabs.tools")}</Tab>,
            ...(isProxyAdmin ? [<Tab key="settings">{t("mcpManagement.tabs.settings")}</Tab>] : []),
          ]}
        </TabList>
        ...
        <TabPanels>
          ...
          <TabPanel>
            <Grid numItems={1} numItemsSm={2} numItemsLg={3} className="gap-4">
              <Card className="p-4">
                <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                  {t("mcpManagement.fields.transport")}
                </Text>
                <div className="mt-3">
                  {getTransportBadge(
              ...
              <Card className="p-4">
                <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                  {t("mcpManagement.fields.authentication")}
                </Text>
              ...
              <Card className="p-4">
                <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                  {t("mcpManagement.fields.hostUrl")}
                </Text>
            ...
            <Card className="mt-4 p-4">
              <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                {t("mcpManagement.fields.costConfiguration")}
              </Text>
              <div className="mt-3">
          ...
          <TabPanel>
            <Card>
              <div className="flex justify-between items-center mb-4">
                <Title>{t("mcpManagement.detail.settingsTitle")}</Title>
                {editing ? null : (
                  <Button variant="light" onClick={() => setEditing(true)}>
                    {t("mcpManagement.actions.edit")}
                  </Button>
                )}
              </div>
              {editing ? (
                ...
                <div className="divide-y divide-gray-100">
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.serverName")}</Text>
                    <div className="col-span-2 text-sm text-gray-900">
                      {mcpServer.server_name || <span className="text-gray-400">—</span>}
                    </div>
                  </div>
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.alias")}</Text>
                    <div className="col-span-2 text-sm font-mono text-gray-900">
                      {mcpServer.alias || <span className="text-gray-400">—</span>}
                    </div>
                  </div>
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.description")}</Text>
                    <div className="col-span-2 text-sm text-gray-900">
                  ...
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.transport")}</Text>
                  ...
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.extraHeaders")}</Text>
                    <div className="col-span-2 text-sm text-gray-900">
                  ...
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.allowAllKeys")}</Text>
                    <div className="col-span-2">
                      {mcpServer.allow_all_keys ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 rounded-full border border-green-200 text-xs font-medium">
                          ...
                          {t("mcpManagement.values.enabled")}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 text-gray-600 rounded-full border border-gray-200 text-xs font-medium">
                          {t("mcpManagement.values.disabled")}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.networkAccess")}</Text>
                    <div className="col-span-2">
                      {mcpServer.available_on_public_internet ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 rounded-full border border-green-200 text-xs font-medium">
                          <span className="h-1.5 w-1.5 rounded-full bg-green-500"></span>
                          {t("mcpManagement.values.public")}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-orange-50 text-orange-700 rounded-full border border-orange-200 text-xs font-medium">
                          ...
                          {t("mcpManagement.values.internalOnly")}
                        </span>
                      )}
                    </div>
                  </div>
                  {handleAuth(mcpServer.auth_type) === "oauth2" && (
                    <div className="py-3 grid grid-cols-3 gap-4">
                      <Text className="text-sm font-medium text-gray-500">
                        {t("mcpManagement.fields.delegateAuth")}
                      </Text>
                      <div className="col-span-2">
                        {mcpServer.delegate_auth_to_upstream ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 rounded-full border border-green-200 text-xs font-medium">
                            <span className="h-1.5 w-1.5 rounded-full bg-green-500"></span>
                            {t("mcpManagement.values.pkceEnabled")}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 text-gray-600 rounded-full border border-gray-200 text-xs font-medium">
                            {t("mcpManagement.values.disabled")}
                  ...
                  {handleAuth(mcpServer.auth_type) !== "oauth2" &&
                    ...
                    mcpServer.extra_headers.some(
                      ...
                      <div className="py-3 grid grid-cols-3 gap-4">
                        <Text className="text-sm font-medium text-gray-500">
                          {t("mcpManagement.fields.oauthPassThrough")}
                        </Text>
                        <div className="col-span-2">
                  ...
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.accessGroups")}</Text>
                    <div className="col-span-2">
                  ...
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.allowedTools")}</Text>
                    <div className="col-span-2">
                      {mcpServer.allowed_tools && mcpServer.allowed_tools.length > 0 ? (
                        ...
                        <span className="inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-sm bg-green-50 text-green-700 border border-green-200">
                          {t("mcpManagement.values.allTools")}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="py-3 grid grid-cols-3 gap-4">
                    <Text className="text-sm font-medium text-gray-500">{t("mcpManagement.fields.cost")}</Text>
                    <div className="col-span-2">
                      <MCPServerCostDisplay costConfig={mcpServer.mcp_info?.mcp_server_cost_info} />
                    </div>
                  </div>
