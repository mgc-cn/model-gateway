...
function ConfirmDialog({ action, serverName, isCurrentlyActive, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  const rejectBody = isCurrentlyActive
    ? t("mcpManagement.submissions.rejectActiveDescription")
    : t("mcpManagement.submissions.rejectDescription");
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <h3 className="text-base font-semibold text-gray-900 mb-1">
          {isApprove ? t("mcpManagement.submissions.approveTitle") : t("mcpManagement.submissions.rejectTitle")}
        </h3>
        <p className="text-sm text-gray-500 mb-4">
          {t("mcpManagement.submissions.confirm", {
            action: isApprove ? t("mcpManagement.submissions.approve") : t("mcpManagement.submissions.reject"),
            name: serverName,
          })}{" "}
          {isApprove ? t("mcpManagement.submissions.approveDescription") : rejectBody}
        </p>
        {!isApprove && (
          <textarea
            placeholder={t("mcpManagement.submissions.rejectionPlaceholder")}
            value={reviewNotes}
        ...
        <div className="flex gap-3">
          ...
          >
            {t("mcpManagement.actions.cancel")}
          </button>
          <button
            type="button"
          ...
          >
            {isApprove ? t("mcpManagement.submissions.approve") : t("mcpManagement.submissions.reject")}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="mb-5 border border-gray-200 rounded-lg bg-white overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-800">{t("mcpManagement.submissions.rules")}</span>
          {activeLabels.length > 0 ? (
            <span className="text-xs text-gray-500">
              ({t("mcpManagement.submissions.requiredCount", { count: activeLabels.length })})
            </span>
          ) : (
            <span className="text-xs text-gray-400 italic">{t("mcpManagement.submissions.noRules")}</span>
          )}
        </div>
            ...
            <div className="flex flex-wrap gap-1.5 max-w-md">
              {activeLabels.map((f) => (
                ...
                >
                  ...
                  {t(`mcpManagement.submissions.fields.${f.key}.label`, { defaultValue: f.label })}
                </span>
              ))}
            </div>
          )}
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pt-4 pb-4">
          <p className="text-xs text-gray-500 mb-4">{t("mcpManagement.submissions.rulesHelp")}</p>
          <div className="grid grid-cols-2 gap-x-8 gap-y-5">
            {FIELD_GROUPS.map((group) => (
              <div key={group.label}>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                  {t(`mcpManagement.submissions.groups.${group.label.toLowerCase()}`, { defaultValue: group.label })}
                </div>
                      ...
                      <label key={field.key} className="flex items-start gap-2.5 cursor-pointer group">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-800 group-hover:text-blue-700 transition-colors">
                            {t(`mcpManagement.submissions.fields.${field.key}.label`, { defaultValue: field.label })}
                          </div>
                          <div className="text-xs text-gray-400">
                            {t(`mcpManagement.submissions.fields.${field.key}.help`, {
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            >
              {isSaving ? t("mcpManagement.submissions.saving") : t("mcpManagement.submissions.saveRules")}
            </button>
            <button
              type="button"
...
function MCPServerCard({ server, onApprove, onReject, requiredFields }: MCPServerCardProps) {
  ...
  const checks = MCP_REQUIRED_FIELD_DEFS.filter((f) => requiredFields.includes(f.key)).map((f) => ({
    key: f.key,
    label: t(`mcpManagement.submissions.fields.${f.key}.label`, { defaultValue: f.label }),
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              >
                ...
                {t(
                  `mcpManagement.submissions.status.${approvalStatus === "pending_review" ? "pending" : approvalStatus}`,
                )}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
              <span>
                {t("mcpManagement.submissions.transport")}{" "}
                <span className="text-gray-600">{server.transport ?? "sse"}</span>
              </span>
              <span>·</span>
              <span>
                {t("mcpManagement.submissions.submittedBy")}{" "}
                <span className="text-gray-600">{server.submitted_by ?? "—"}</span>
              </span>
              <span>·</span>
              <span>{server.submitted_at ? new Date(server.submitted_at).toLocaleDateString(i18n.language) : "—"}</span>
            </div>
            {approvalStatus === "rejected" && server.review_notes && (
              <p className="text-xs text-red-600 mt-1.5">
                {t("mcpManagement.submissions.rejectionReason")} {server.review_notes}
              </p>
            )}
          </div>
          ...
          {checks.length === 0 && approvalStatus !== "rejected" && (
            <div className="flex items-center gap-2 shrink-0">
              {approvalStatus !== "active" && (
                ...
                >
                  {t("mcpManagement.submissions.approve")}
                </button>
              )}
              <button
                type="button"
              ...
              >
                {t("mcpManagement.submissions.reject")}
              </button>
            </div>
          )}
          {checks.length === 0 && approvalStatus === "rejected" && (
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="text-xs bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-md transition-colors font-medium"
              >
                {t("mcpManagement.submissions.reapprove")}
              </button>
            </div>
          )}
        </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-sm font-semibold leading-tight ${allPassed ? "text-green-800" : "text-red-800"}`}>
                {allPassed
                  ? t("mcpManagement.submissions.allChecksPassed")
                  : t("mcpManagement.submissions.checksFailed", { count: failCount })}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">
                {t("mcpManagement.submissions.checkSummary", { passing: passCount, failing: failCount })}
              </div>
            </div>
            ...
            {checks.map((c) => (
              <div key={c.key} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span className={`text-xs ${c.passed ? "text-green-600" : "text-red-500"}`}>
                  {c.passed ? t("mcpManagement.submissions.passes") : t("mcpManagement.submissions.missing")}
                </span>
              </div>
            ))}
          </div>
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("mcpManagement.submissions.loadFailed"));
    } finally {
      setIsLoading(false);
    }
  }, [accessToken, t]);
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("mcpManagement.submissions.rulesSaved"));
    } catch {
      NotificationsManager.fromBackend(t("mcpManagement.submissions.rulesSaveFailed"));
    } finally {
  ...
  async function handleApprove(serverId: string, serverName: string) {
    ...
    try {
      ...
      await fetchData();
      NotificationsManager.success(t("mcpManagement.submissions.approved", { name: serverName }));
    } catch {
      NotificationsManager.fromBackend(t("mcpManagement.submissions.approveFailed"));
    } finally {
  ...
  async function handleReject(serverId: string, serverName: string, reviewNotes?: string) {
    ...
    try {
      ...
      await fetchData();
      NotificationsManager.success(t("mcpManagement.submissions.rejected", { name: serverName }));
    } catch {
      NotificationsManager.fromBackend(t("mcpManagement.submissions.rejectFailed"));
    } finally {
      setConfirmAction(null);
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard label={t("mcpManagement.submissions.total")} value={summary.total} color="text-gray-900" />
        <StatCard
          label={t("mcpManagement.submissions.status.pending")}
          value={summary.pending_review}
          color="text-yellow-600"
        />
        <StatCard label={t("mcpManagement.submissions.status.active")} value={summary.active} color="text-green-600" />
        <StatCard
          label={t("mcpManagement.submissions.status.rejected")}
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            type="text"
            placeholder={t("mcpManagement.submissions.search")}
            value={search}
        ...
        >
          <option value="all">{t("mcpManagement.submissions.status.all")}</option>
          <option value="pending_review">{t("mcpManagement.submissions.status.pending")}</option>
          <option value="active">{t("mcpManagement.submissions.status.active")}</option>
          <option value="rejected">{t("mcpManagement.submissions.status.rejected")}</option>
        </select>
      </div>

      <div className="space-y-3">
        {isLoading && (
          <div className="text-center py-12 text-gray-500 text-sm">{t("mcpManagement.submissions.loading")}</div>
        )}
        {error && <div className="text-center py-12 text-red-600 text-sm">{error}</div>}
        {!isLoading && !error && filtered.length === 0 && (
          <div className="text-center py-12 text-gray-400 text-sm">{t("mcpManagement.submissions.empty")}</div>
        )}
        {!isLoading &&
          !error &&
          filtered.map((server) => (
