...
const MCPServers: React.FC<MCPServerProps> = ({ accessToken, userRole, userID }) => {
  ...
  const sortOptions: { value: SortKey; label: string }[] = [
    { value: "created_desc", label: t("mcpManagement.sort.created") },
    { value: "updated_desc", label: t("mcpManagement.sort.updated") },
    { value: "name_asc", label: t("mcpManagement.sort.name") },
    { value: "health", label: t("mcpManagement.sort.health") },
  ];
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("mcpManagement.delete.success"));
  ...
  if (!accessToken || !userRole || !userID) {
    return <div className="p-6 text-center text-gray-500">{t("mcpManagement.states.missingAuth")}</div>;
  }
  ...
  return (
    <div className="w-full h-full p-6">
      <Modal
        ...
        title={t("mcpManagement.delete.title")}
        onOk={confirmDelete}
        okText={isDeletingServer ? t("mcpManagement.actions.deleting") : t("mcpManagement.actions.delete")}
        onCancel={cancelDelete}
        cancelText={t("mcpManagement.actions.cancel")}
        cancelButtonProps={{ disabled: isDeletingServer }}
        okButtonProps={{ danger: true }}
        confirmLoading={isDeletingServer}
      >
        <div className="space-y-4">
          <AntdText className="text-gray-600">{t("mcpManagement.delete.warning")}</AntdText>
            ...
            <div className="mt-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <Descriptions column={1} size="small" colon={false}>
                {serverToDelete.server_name && (
                  <Descriptions.Item
                    label={<span className="text-gray-500 text-sm">{t("mcpManagement.fields.name")}</span>}
                  >
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            <Title>{t("mcpManagement.title")}</Title>
          ...
          <Text className="text-tremor-content mt-1">{t("mcpManagement.description")}</Text>
        </div>
        <div className="flex items-center gap-2">
          {isAdminRole(userRole) && (
            <Button className="shrink-0" onClick={() => setDiscoveryVisible(true)}>
              {t("mcpManagement.actions.add")}
            </Button>
          )}
          {!isAdminRole(userRole) && (
            <Button
              ...
              variant="secondary"
            >
              {t("mcpManagement.actions.submit")}
            </Button>
          )}
        </div>
      </div>
      ...
      <TabGroup className="w-full h-full">
        <TabList className="flex justify-between mt-2 w-full items-center">
          <div className="flex">
            <Tab>{t("mcpManagement.tabs.all")}</Tab>
            <Tab>{t("mcpManagement.tabs.toolsets")}</Tab>
            <Tab>{t("mcpManagement.tabs.connect")}</Tab>
            {isAdminRole(userRole) && <Tab>{t("mcpManagement.tabs.semanticFilter")}</Tab>}
            {isAdminRole(userRole) && <Tab>{t("mcpManagement.tabs.network")}</Tab>}
            {isAdminRole(userRole) && (
              <Tab>
                <span className="flex items-center gap-2">
                  {t("mcpManagement.tabs.submissions")} <NewBadge />
                </span>
              </Tab>
            )}
          </div>
              ...
              <div className="w-full h-full">
                    ...
                    <div className="flex items-center gap-6 bg-white rounded-lg px-4 py-3 border border-gray-200">
                      <div className="flex items-center gap-2">
                        <Text className="text-sm font-medium text-gray-600 whitespace-nowrap">
                          {t("mcpManagement.filters.team")}
                        </Text>
                        <Select value={selectedTeam} onChange={handleTeamChange} style={{ width: 220 }} size="middle">
                          <Option value="all">
                            <span className="font-medium">
                              {isInternalUser
                                ? t("mcpManagement.filters.allAvailable")
                                : t("mcpManagement.filters.allServers")}
                            </span>
                          </Option>
                          <Option value="personal">
                            <span className="font-medium">{t("mcpManagement.filters.personal")}</span>
                          </Option>
                      ...
                      <div className="flex items-center gap-2">
                        <Text className="text-sm font-medium text-gray-600 whitespace-nowrap">
                          {t("mcpManagement.filters.accessGroup")}
                          <Tooltip title={t("mcpManagement.filters.accessGroupHelp")}>
                        ...
                        >
                          <Option value="all">
                            <span className="font-medium">{t("mcpManagement.filters.allAccessGroups")}</span>
                          </Option>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <Input
                    ...
                    placeholder={t("mcpManagement.filters.search")}
                    value={searchQuery}
                  ...
                  <div className="ml-auto text-xs text-gray-500">
                    {t("mcpManagement.filters.count", {
                      shown: displayedServers.length,
                ...
                <div className="mt-4 w-full">
                  {isLoadingServers ? (
                    <div className="flex items-center justify-center rounded-lg border border-dashed border-gray-200 bg-white p-12">
                      <Spin tip={t("mcpManagement.states.loading")} />
                    </div>
                    ...
                    <div className="rounded-lg border border-dashed border-gray-200 bg-white p-12">
                      <Empty
                        description={
                          filteredServers.length === 0
                            ? t("mcpManagement.states.empty")
                            : t("mcpManagement.states.noMatches")
                        }
                      />
                    </div>
                  ) : (
