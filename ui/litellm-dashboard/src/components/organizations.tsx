...
const OrganizationsTable: React.FC<OrganizationsTableProps> = ({
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(i18n.t("organizationManagement.notifications.deleted"));
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      NotificationsManager.success(i18n.t("organizationManagement.notifications.created"));
  ...
  if (!premiumUser) {
    return (
      <div>
        <Text>
          {t("organizationManagement.enterprise.prefix")}{" "}
          <a href="https://www.litellm.ai/#pricing" target="_blank" rel="noopener noreferrer">
            {t("organizationManagement.enterprise.link")}
          </a>
          .
        </Text>
      </div>
  ...
  return (
    <div className="w-full mx-4 h-[75vh]">
        ...
        <Col numColSpan={1} className="flex flex-col gap-2">
          {(userRole === "Admin" || userRole === "Org Admin") && (
            <Button className="w-fit" onClick={() => setIsOrgModalVisible(true)}>
              {t("organizationManagement.createNew")}
            </Button>
          )}
          {selectedOrgId ? (
            ...
            <TabGroup className="gap-2 h-[75vh] w-full">
              <TabList className="flex justify-between mt-2 w-full items-center">
                <div className="flex">
                  <Tab>{t("organizationManagement.yourOrganizations")}</Tab>
                </div>
                <div className="flex items-center space-x-2">
                  {lastRefreshed && <Text>{t("organizationManagement.lastRefreshed", { time: lastRefreshed })}</Text>}
                  <Icon
                    icon={RefreshIcon}
                    variant="shadow"
                    size="xs"
              ...
              <TabPanels>
                <TabPanel>
                  <Text>{t("organizationManagement.viewDetailsHelp")}</Text>
                    ...
                    <Col numColSpan={1}>
                      <Card className="w-full mx-auto flex-auto overflow-hidden overflow-y-auto max-h-[50vh]">
                        ...
                        <Table>
                          <TableHead>
                            <TableRow>
                              <TableHeaderCell>{t("organizationManagement.fields.id")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.name")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.created")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.spend")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.budget")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.models")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.rateLimits")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.info")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationManagement.fields.actions")}</TableHeaderCell>
                            </TableRow>
                          </TableHead>
                              ...
                              ? organizations
                                  ...
                                  .map((org: Organization) => (
                                    <TableRow key={org.organization_id}>
                                      ...
                                      <TableCell>
                                        {org.created_at ? new Date(org.created_at).toLocaleDateString(locale) : "N/A"}
                                      </TableCell>
                                            ...
                                            <div className="flex flex-col">
                                              {org.models.length === 0 ? (
                                                <Badge size={"xs"} className="mb-1" color="red">
                                                  <Text>{t("organizationManagement.allProxyModels")}</Text>
                                                </Badge>
                                              ) : (
                                                <>
                                                      ...
                                                      {org.models.length > 3 &&
                                                        !expandedAccordions[org.organization_id || ""] && (
                                                          <Badge size={"xs"} color="gray" className="cursor-pointer">
                                                            <Text>
                                                              +{org.models.length - 3}{" "}
                                                              {t("organizationManagement.moreModels", {
                                                          ...
                                                          {org.models.slice(3).map((model, index) =>
                                                            model === "all-proxy-models" ? (
                                                              <Badge key={index + 3} size={"xs"} color="red">
                                                                <Text>
                                                                  {t("organizationManagement.allProxyModels")}
                                                                </Text>
                                                              </Badge>
                                                            ) : (
                                      ...
                                      <TableCell>
                                        <Text>
                                          {t("organizationManagement.memberCount", { count: org.members?.length || 0 })}
                                        </Text>
                                      </TableCell>
                                      <TableCell>
                                        {userRole === "Admin" && (
                                          <>
                                            <TableIconActionButton
                                              variant="Edit"
                                              tooltipText={t("organizationManagement.actions.edit")}
                                              onClick={() => {
                                            ...
                                            <TableIconActionButton
                                              variant="Delete"
                                              tooltipText={t("organizationManagement.actions.delete")}
      ...
      <Modal
        title={t("organizationManagement.create")}
        ...
        <Form form={form} onFinish={handleCreate} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} labelAlign="left">
          ...
          <Form.Item label={t("organizationManagement.form.resetBudget")} name="budget_duration">
            <Select2 placeholder="n/a">
              <Select2.Option value="24h">{t("organizationManagement.form.daily")}</Select2.Option>
              <Select2.Option value="7d">{t("organizationManagement.form.weekly")}</Select2.Option>
              <Select2.Option value="30d">{t("organizationManagement.form.monthly")}</Select2.Option>
            </Select2>
          </Form.Item>
          ...
          <Form.Item
            label={
              <span>
                {t("organizationManagement.form.allowedVectorStores")}{" "}
                <Tooltip title={t("organizationManagement.form.vectorStoresHelp")}>
            ...
            className="mt-4"
            help={t("organizationManagement.form.vectorStoresHelp")}
          >
            <VectorStoreSelector
              onChange={(values) => form.setFieldValue("allowed_vector_store_ids", values)}
              value={form.getFieldValue("allowed_vector_store_ids")}
              accessToken={accessToken || ""}
              placeholder={t("organizationManagement.form.vectorStoresPlaceholder")}
            />
          </Form.Item>
          ...
          <Form.Item
            label={
              <span>
                {t("organizationManagement.form.allowedMcp")}{" "}
                <Tooltip title={t("organizationManagement.form.mcpHelp")}>
                  <InfoCircleOutlined style={{ marginLeft: "4px" }} />
                </Tooltip>
              </span>
            }
            ...
            className="mt-4"
            help={t("organizationManagement.form.mcpHelp")}
          >
            <MCPServerSelector
              onChange={(values) => form.setFieldValue("allowed_mcp_servers_and_groups", values)}
              value={form.getFieldValue("allowed_mcp_servers_and_groups")}
              accessToken={accessToken || ""}
              placeholder={t("organizationManagement.form.mcpPlaceholder")}
            />
          </Form.Item>
          ...
          <div style={{ textAlign: "right", marginTop: "10px" }}>
            <Button type="submit">{t("organizationManagement.create")}</Button>
          </div>
        </Form>
      </Modal>
      ...
      <DeleteResourceModal
        ...
        title={t("organizationManagement.delete.title")}
        message={t("organizationManagement.delete.message")}
        resourceInformationTitle={t("organizationManagement.delete.resourceTitle")}
        resourceInformation={[{ label: t("organizationManagement.fields.id"), value: orgToDelete, code: true }]}
