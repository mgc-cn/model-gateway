...
const VectorStoreInfoView: React.FC<VectorStoreInfoViewProps> = ({
  ...
  const fetchVectorStoreDetails = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("toolManagement.vectors.fetchDetailFailed", { error: String(error) }));
    }
  };
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      NotificationsManager.success(t("toolManagement.vectors.updated"));
      setIsEditing(false);
      fetchVectorStoreDetails();
    } catch (error) {
      console.error("Error updating vector store:", error);
      NotificationsManager.fromBackend(t("toolManagement.vectors.updateFailed", { error: String(error) }));
    }
  };

  if (!vectorStoreDetails) {
    return <div>{t("toolManagement.common.loading")}</div>;
  }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <Button icon={ArrowLeftIcon} variant="light" className="mb-4" onClick={onClose}>
            {t("toolManagement.vectors.back")}
          </Button>
          <Title>
            {t("toolManagement.vectors.vectorId")}: {vectorStoreDetails.vector_store_id}
          </Title>
          <Text className="text-gray-500">
            {vectorStoreDetails.vector_store_description || t("toolManagement.vectors.noDescription")}
          </Text>
        </div>
        {is_admin && !isEditing && (
          <Button onClick={() => setIsEditing(true)}>{t("toolManagement.vectors.edit")}</Button>
        )}
      </div>

      <TabGroup>
        <TabList className="mb-6">
          <Tab>{t("toolManagement.vectors.detailsTab")}</Tab>
          <Tab>{t("toolManagement.vectors.testDetailsTab")}</Tab>
        </TabList>
            ...
            {isEditing ? (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <Title>{t("toolManagement.vectors.edit")}</Title>
                </div>
                <Card>
                  <Form form={form} onFinish={handleSave} layout="vertical" initialValues={vectorStoreDetails}>
                    <Form.Item
                      label={t("toolManagement.vectors.vectorId")}
                      name="vector_store_id"
                      rules={[{ required: true, message: t("toolManagement.vectors.idRequired") }]}
                    >
                      <Input disabled />
                    </Form.Item>

                    <Form.Item label={t("toolManagement.vectors.name")} name="vector_store_name">
                      <Input />
                    </Form.Item>
                    ...
                    {/* Credentials */}
                    <div className="mb-4">
                      <Text className="text-sm text-gray-500 mb-2">{t("toolManagement.vectors.credentialsHelp")}</Text>
                    </div>

                    <Form.Item label={t("toolManagement.vectors.existingCredentials")} name="litellm_credential_name">
                      <Select2
                        showSearch
                    ...
                    <Form.Item
                      label={
                        <span>
                          ...
                          <Tooltip title={t("toolManagement.vectors.metadataHelp")}>
                            <InfoCircleOutlined style={{ marginLeft: "4px" }} />
                          </Tooltip>
                        </span>
                      }
                    ...
                    <div className="flex justify-end space-x-2">
                      ...
                      <AntButton type="primary" htmlType="submit">
                        {t("toolManagement.vectors.saveChanges")}
                      </AntButton>
                    </div>
                  </Form>
                </Card>
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <Title>{t("toolManagement.vectors.details")}</Title>
                  {is_admin && <Button onClick={() => setIsEditing(true)}>{t("toolManagement.vectors.edit")}</Button>}
                </div>
                <Card>
                  <div className="space-y-4">
                    ...
                    <div>
                      <Text className="font-medium">{t("toolManagement.common.name")}</Text>
                      <Text>{vectorStoreDetails.vector_store_name || "-"}</Text>
                    </div>
                    <div>
                      <Text className="font-medium">{t("toolManagement.common.description")}</Text>
                      <Text>{vectorStoreDetails.vector_store_description || "-"}</Text>
                    </div>
                    <div>
                      <Text className="font-medium">{t("toolManagement.common.provider")}</Text>
                    ...
                    <div>
                      <Text className="font-medium">{t("toolManagement.vectors.metadata")}</Text>
                      <div className="bg-gray-50 p-3 rounded-sm mt-2 font-mono text-xs overflow-auto max-h-48">
                        <pre>{metadataString}</pre>
                      </div>
                    </div>
                    <div>
                      <Text className="font-medium">{t("toolManagement.vectors.createdLabel")}</Text>
                      <Text>
                    ...
                    <div>
                      <Text className="font-medium">{t("toolManagement.vectors.updatedLabel")}</Text>
                      <Text>
