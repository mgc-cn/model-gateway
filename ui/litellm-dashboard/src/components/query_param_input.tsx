    ...
    <div>
      {pairs.map(([key, val], index) => (
        <Space key={index} style={{ display: "flex", marginBottom: 8 }} align="center">
          <TextInput
            placeholder={t("modelsAndEndpoints.passThrough.queryParams.namePlaceholder")}
            value={key}
            onChange={(e) => handleChange(index, e.target.value, val)}
          />
          <TextInput
            placeholder={t("modelsAndEndpoints.passThrough.queryParams.valuePlaceholder")}
            value={val}
      ...
      <Button type="dashed" onClick={handleAdd} icon={<PlusOutlined />}>
        {t("modelsAndEndpoints.passThrough.queryParams.add")}
      </Button>
    </div>
  );
};
