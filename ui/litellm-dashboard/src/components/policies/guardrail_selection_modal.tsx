...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const guardrailsInfo: GuardrailInfo[] = (template?.guardrailDefinitions || []).map((def: any) => ({
    ...
    description: def.guardrail_info?.description || t("policyManagement.noDescription"),
    ...
    <Modal
      title={
        <div>
          <div className="flex items-center gap-2">
            ...
            {progressInfo && (
              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-600 border border-blue-100">
                {t("policyManagement.guardrailModal.templateProgress", {
          ...
          <p className="text-sm text-gray-500 font-normal mt-1">{t("policyManagement.guardrailModal.description")}</p>
        </div>
      }
      open={visible}
      onCancel={onCancel}
      ...
      footer={[
        ...
        >
          {selectedCount > 0
            ? t("policyManagement.guardrailModal.createAndUse", { count: selectedCount })
            : t("policyManagement.templates.useTemplate")}
        </Button>,
      ]}
    >
      <div className="py-4">
          ...
          <div className="flex-1">
            <div className="text-sm">
              <span className="font-medium text-gray-900">
                {t("policyManagement.guardrailModal.total", { count: guardrailsInfo.length })}
              </span>
              <span className="text-gray-600 mx-2">•</span>
              <span className="text-green-600 font-medium">
                {t("policyManagement.guardrailModal.newCount", { count: newGuardrailsCount })}
              </span>
              {existingCount > 0 && (
                <>
                  <span className="text-gray-600 mx-2">•</span>
                  <span className="text-gray-600">
                    {t("policyManagement.guardrailModal.existingCount", { count: existingCount })}
                  </span>
                </>
              )}
            </div>
          ...
          {newGuardrailsCount > 0 && (
            <div className="flex gap-2">
              <Button size="small" onClick={handleSelectAll}>
                {t("policyManagement.guardrailModal.selectAllNew")}
              </Button>
              <Button size="small" onClick={handleDeselectAll}>
                {t("policyManagement.guardrailModal.deselectAll")}
              </Button>
            </div>
          )}
        </div>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {guardrail.alreadyExists && (
                      <Tag color="green" className="text-xs">
                        {t("policyManagement.guardrailModal.alreadyExists")}
                      </Tag>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    {guardrail.definition?.litellm_params?.patterns && (
                      <Tag className="text-xs" color="purple">
                        {t("policyManagement.guardrailModal.patternCount", {
                    ...
                    {guardrail.definition?.litellm_params?.categories && (
                      <Tag className="text-xs" color="orange">
                        {t("policyManagement.guardrailModal.categoryCount", {
        ...
        {guardrailsInfo.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            <p>{t("policyManagement.guardrailModal.empty")}</p>
            <p className="text-sm mt-2">{t("policyManagement.guardrailModal.emptyHelp")}</p>
          </div>
        )}
            ...
            <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-purple-900 text-sm">
                  {t("policyManagement.guardrailModal.discoveredCompetitors", {
              ...
              <p className="text-xs text-purple-600 mt-2">{t("policyManagement.guardrailModal.competitorsHelp")}</p>
            </div>
          </>
        )}
        ...
        <div className="text-sm text-gray-600">
          {selectedCount > 0 ? (
            <p>{t("policyManagement.guardrailModal.willCreate", { count: selectedCount })}</p>
          ) : existingCount > 0 ? (
            <p className="text-green-600">{t("policyManagement.guardrailModal.allExist")}</p>
          ) : (
            <p className="text-orange-600">{t("policyManagement.guardrailModal.selectRequired")}</p>
          )}
        </div>
      </div>
    </Modal>
