...
const MemberModal = <T extends BaseMember>({
    ...
    switch (field.type) {
      ...
      case "numerical":
        return (
          <NumericalInput
            ...
            min={field.min || 0}
            style={{ width: "100%" }}
            placeholder={field.placeholder || t("common.memberForm.numericalPlaceholder")}
          />
        );
      case "select":
        return (
      ...
      case "multi-select":
        return (
          <Select
            mode="multiple"
            placeholder={field.placeholder || t("common.memberForm.selectOptions")}
  ...
  return (
    <Modal
      title={config.title || (mode === "add" ? t("common.memberForm.addMember") : t("common.memberForm.editMember"))}
      open={visible}
      width={1000}
      footer={null}
      onCancel={onCancel}
    >
      <Form form={form} onFinish={handleSubmit} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} labelAlign="left">
        {config.showEmail && (
          <Form.Item
            label={t("common.memberForm.email")}
            name="user_email"
            className="mb-4"
            rules={[{ type: "email", message: t("common.memberForm.emailInvalid") }]}
          >
        ...
        {config.showEmail && config.showUserId && (
          <div className="text-center mb-4">
            <Text>{t("common.memberForm.or")}</Text>
          </div>
        )}

        {config.showUserId && (
          <Form.Item label={t("common.memberForm.userId")} name="user_id" className="mb-4">
        ...
        <Form.Item
          label={
            <div className="flex items-center gap-2">
              <span>{t("common.memberForm.role")}</span>
              {mode === "edit" && initialData && (
                <span className="text-gray-500 text-sm">
                  ({t("common.memberForm.current", { role: getRoleLabel(initialData.role) })})
                </span>
              )}
            </div>
          }
          name="role"
          className="mb-4"
          rules={[{ required: true, message: t("common.memberForm.roleRequired") }]}
        >
          <Select>
            {mode === "edit" && initialData
              ? [
        ...
        <div className="text-right mt-6">
          ...
          <AntButton type="default" htmlType="submit" loading={isSubmitting}>
            {mode === "add"
              ? isSubmitting
                ? t("common.memberForm.adding")
                : t("common.memberForm.addMember")
              : isSubmitting
                ? t("common.memberForm.saving")
                : t("common.memberForm.saveChanges")}
          </AntButton>
        </div>
      </Form>
    </Modal>
