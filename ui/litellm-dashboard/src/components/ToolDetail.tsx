...
export function ToolDetail({ toolName, onBack, accessToken }: ToolDetailProps) {
  ...
  const handleInputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(t("toolManagement.policies.updateInputFailed", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
  ...
  const handleOutputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(t("toolManagement.policies.updateOutputFailed", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
  ...
  const handleAddOverride = useCallback(async () => {
    ...
    try {
      ...
      alert(t("toolManagement.policies.addOverrideFailed", { error: e instanceof Error ? e.message : String(e) }));
    } finally {
  ...
  const handleRemoveOverride = useCallback(
    async (override: ToolPolicyOverrideRow) => {
      ...
      try {
        ...
        alert(t("toolManagement.policies.removeOverrideFailed", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
        setOverrideSaving(false);
      }
    },
    ...
    return (
      <div>
        <Button type="link" icon={<ArrowLeftOutlined />} onClick={onBack} className="pl-0 mb-4">
          {t("toolManagement.policies.back")}
        </Button>
        <p className="text-red-600">{t("toolManagement.policies.detailLoadFailed")}</p>
      </div>
    );
  }
          ...
          <div>
            <div className="flex items-center gap-3 mb-1 flex-wrap">
              ...
              <span className="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200">
                {t("toolManagement.policies.callCount", { count: tool.call_count ?? 0 })}
              </span>
            </div>
            <dl className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-600">
              {tool.user_agent && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium text-gray-500 whitespace-nowrap">
                    {t("toolManagement.policies.userAgent")}:
                  </dt>
              ...
              {tool.created_at && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium text-gray-500 whitespace-nowrap">
                    {t("toolManagement.policies.firstDiscovered")}
                  </dt>
                  <dd>{new Date(tool.created_at).toLocaleString(i18n.language)}</dd>
                </div>
              )}
              {tool.last_used_at && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium text-gray-500 whitespace-nowrap">
                    {t("toolManagement.policies.lastUsed")}
                  </dt>
                  <dd>{new Date(tool.last_used_at).toLocaleString(i18n.language)}</dd>
                </div>
              )}
            </dl>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <section className="bg-white rounded-lg border border-gray-200 p-5 shadow-xs">
            <h2 className="text-sm font-semibold text-gray-700 mb-1">{t("toolManagement.policies.input")}</h2>
            <p className="text-xs text-gray-500 mb-3">
              {i18n.language.startsWith("zh")
                ? t("toolManagement.policies.inputDescription")
                : inputDesc ?? t("toolManagement.policies.inputDescription")}
            </p>
            <PolicySelect
          ...
          <section className="bg-white rounded-lg border border-gray-200 p-5 shadow-xs">
            <h2 className="text-sm font-semibold text-gray-700 mb-1">{t("toolManagement.policies.output")}</h2>
            <p className="text-xs text-gray-500 mb-3">
              {i18n.language.startsWith("zh")
                ? t("toolManagement.policies.outputDescription")
                : outputDesc ?? t("toolManagement.policies.outputDescription")}
            </p>
            <PolicySelect
        ...
        {overrides.length > 0 && (
          <section className="bg-white rounded-lg border border-gray-200 p-5 shadow-xs">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">{t("toolManagement.policies.blockedFor")}</h2>
              ...
              {overrides.map((ov) => (
                <li key={ov.override_id} className="flex items-center justify-between px-3 py-2.5 text-sm">
                  <span className="text-gray-700">
                    {ov.team_id ? `${t("toolManagement.policies.teamLabel")}: ${ov.team_id}` : ""}
                    {ov.team_id && ov.key_hash ? " · " : ""}
                    {ov.key_hash
                      ? `${t("toolManagement.policies.keyLabel")}: ${ov.key_alias || ov.key_hash.substring(0, 8)}`
                      : ""}
                    {!ov.team_id && !ov.key_hash ? "—" : ""}
                  </span>
                  <Button
                  ...
                  >
                    {t("toolManagement.policies.remove")}
                  </Button>
                </li>
              ))}
            </ul>
        ...
        <section className="bg-white rounded-lg border border-gray-200 p-5 shadow-xs">
          <h2 className="text-sm font-semibold text-gray-700 mb-3">{t("toolManagement.policies.blockFor")}</h2>
          <div className="flex flex-col gap-4 max-w-md">
            <div>
              <span className="text-sm font-medium text-gray-700 block mb-2">{t("toolManagement.policies.scope")}</span>
              <div className="flex items-center gap-6">
                <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                  ...
                  />
                  {t("toolManagement.policies.teamLabel")}
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                  <input
                    type="radio"
                    ...
                    className="align-middle"
                  />
                  {t("toolManagement.policies.keyLabel")}
                </label>
              </div>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-700 block mb-2">
                {blockScope === "team" ? t("toolManagement.policies.teamLabel") : t("toolManagement.policies.keyLabel")}
              </span>
              {blockScope === "team" ? (
                <TeamDropdown value={blockTeamId ?? undefined} onChange={(id) => setBlockTeamId(id || null)} />
              ) : (
                <Select
                  placeholder={t("toolManagement.policies.selectKey")}
                  allowClear
                  showSearch
            ...
            >
              {t("toolManagement.policies.blockAction", {
                scope:
                  blockScope === "team"
                    ? t("toolManagement.policies.teamLabel")
                    : t("toolManagement.policies.keyLabel"),
        ...
        <section className="bg-white rounded-lg border border-gray-200 p-5 shadow-xs">
          <h2 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
            <HistoryOutlined />
            {t("toolManagement.policies.recentLogs")}
          </h2>
          <LogViewer
