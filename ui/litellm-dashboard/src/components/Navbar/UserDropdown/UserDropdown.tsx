    ...
    <Space direction="vertical" size="small" style={{ width: "100%", padding: "12px" }}>
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        ...
        {premiumUser ? (
          ...
          <Tooltip title={t("account.upgradeTooltip")} placement="left">
            <Tag icon={<CrownOutlined />}>{t("account.standard")}</Tag>
          </Tooltip>
        )}
      </Space>
      ...
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Space>
          <SafetyOutlined />
          <Text type="secondary">{t("account.role")}</Text>
        </Space>
        <Text>{userRole}</Text>
      </Space>
      <Divider style={{ margin: "8px 0" }} />
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text type="secondary">{t("account.hideNewFeatures")}</Text>
        <Switch
          size="small"
      ...
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text type="secondary">{t("account.hidePrompts")}</Text>
        <Switch
          size="small"
      ...
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text type="secondary">{t("account.hideUsage")}</Text>
        <Switch
          size="small"
          ...
          aria-label={t("account.toggleUsage")}
        />
      </Space>
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text type="secondary">{t("account.hideBlog")}</Text>
        <Switch
          size="small"
          ...
          aria-label={t("account.toggleBlog")}
        />
      </Space>
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text type="secondary">{t("account.hideBouncingIcon")}</Text>
        <Switch
          size="small"
  ...
  return (
    ...
    >
      <Button
        ...
        aria-label={`${t("account.menu")}: ${userRole ?? t("account.unknownRole")}; ${t("account.signedInAs")}: ${userEmail || userId || "unknown"}`}
        aria-haspopup="menu"
      >
        <span
