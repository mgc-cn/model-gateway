...
export default function AddFallbacks({ accessToken, value = [], onChange }: AddFallbacksProps) {
  ...
  const handleSaveAll = async () => {
    ...
    if (invalidGroups.length > 0) {
      MessageManager.error(t("routerSettings.fallbacks.incomplete", { count: invalidGroups.length }));
      return;
    }
    ...
    if (onChange) {
      ...
      try {
        ...
        NotificationManager.success(t("routerSettings.fallbacks.added", { count: groups.length }));
        handleCancel();
      } catch (error) {
      ...
      NotificationManager.fromBackend(t("routerSettings.fallbacks.callbackMissing"));
    }
  };
        ...
        {groups.length > 0 && (
          <div className="flex items-center justify-end space-x-3 pt-6 mt-6 border-t border-gray-100">
            <Button type="default" onClick={handleCancel} disabled={isSaving}>
              {t("routerSettings.actions.cancel")}
            </Button>
            <Button
              type="default"
              ...
              loading={isSaving}
            >
              {isSaving ? t("routerSettings.fallbacks.saving") : t("routerSettings.fallbacks.saveAll")}
            </Button>
          </div>
        )}
      </AddFallbacksModal>
