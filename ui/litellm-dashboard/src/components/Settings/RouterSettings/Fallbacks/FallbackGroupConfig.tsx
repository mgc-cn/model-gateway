    ...
    <div className="flex flex-col gap-8 py-4">
      ...
      <div className="relative">
        <label className="block text-sm font-semibold text-gray-700 mb-2">
          {t("routerSettings.fallbacks.primaryModel")} <span className="text-red-500">*</span>
        </label>
        <Select
          className="w-full h-12"
          size="large"
          placeholder={t("routerSettings.fallbacks.selectPrimary")}
        ...
        {!group.primaryModel && (
          <div className="mt-2 flex items-center gap-2 text-amber-600 text-xs bg-amber-50 p-2 rounded-sm">
            ...
            <span>{t("routerSettings.fallbacks.selectPrimaryHint")}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-center -my-4 z-10">
        <div className="bg-indigo-50 text-indigo-500 px-4 py-1 rounded-full text-xs font-bold border border-indigo-100 flex items-center gap-2 shadow-xs">
          ...
          {t("routerSettings.fallbacks.ifFails")}
        </div>
      </div>
      ...
      >
        <label className="block text-sm font-semibold text-gray-700 mb-2">
          {t("routerSettings.fallbacks.chain")} <span className="text-red-500">*</span>
          <span className="text-xs text-gray-500 font-normal ml-2">
            {t("routerSettings.fallbacks.maxAtATime", { max: maxFallbacks })}
          </span>
        </label>
        ...
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
          ...
          <div className="mb-4">
            <Select
              ...
              placeholder={
                canAddMoreFallbacks
                  ? t("routerSettings.fallbacks.selectModels")
                  : t("routerSettings.fallbacks.maximumReached", { max: maxFallbacks })
              }
              ...
              maxTagPlaceholder={(omittedValues) => (
                ...
                >
                  <span>{t("routerSettings.fallbacks.more", { count: omittedValues.length })}</span>
                </Tooltip>
              )}
              showSearch
              filterOption={(input, option) => (option?.label ?? "").toLowerCase().includes(input.toLowerCase())}
            ...
            <p className="text-xs text-gray-500 mt-1 ml-1">
              {canAddMoreFallbacks
                ? t("routerSettings.fallbacks.selectionHelp", {
                    used: group.fallbackModels.length,
                    max: maxFallbacks,
                  })
                : t("routerSettings.fallbacks.maximumHelp", { max: maxFallbacks })}
            </p>
          </div>
          ...
          <div className="space-y-2 min-h-[100px]">
            {group.fallbackModels.length === 0 ? (
              <div className="h-32 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-gray-400">
                <span className="text-sm">{t("routerSettings.fallbacks.noneSelected")}</span>
                <span className="text-xs mt-1">{t("routerSettings.fallbacks.addFromDropdown")}</span>
              </div>
            ) : (
