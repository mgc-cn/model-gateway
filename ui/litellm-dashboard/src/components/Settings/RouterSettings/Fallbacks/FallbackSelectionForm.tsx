...
export function FallbackSelectionForm({
  ...
  const handleRemoveGroup = (targetId: string) => {
    if (groups.length === 1) {
      MessageManager.warning(t("routerSettings.fallbacks.atLeastOneGroup"));
      return;
    }
  ...
  const items = groups.map((group, index) => {
    const label = group.primaryModel ? group.primaryModel : t("routerSettings.fallbacks.group", { number: index + 1 });
    return {
      key: group.id,
      label: label,
  ...
  if (groups.length === 0) {
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
        <p className="text-gray-500 mb-4">{t("routerSettings.fallbacks.noGroups")}</p>
        <Button variant="primary" onClick={handleAddGroup} icon={() => <Plus className="w-4 h-4" />}>
          {t("routerSettings.fallbacks.createFirstGroup")}
        </Button>
      </div>
    );
  }
