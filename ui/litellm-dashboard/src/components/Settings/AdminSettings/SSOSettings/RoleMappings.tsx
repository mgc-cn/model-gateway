...
export default function RoleMappings({ roleMappings }: { roleMappings: RoleMappingsType | undefined }) {
  ...
  const roleMappingsColumns = [
    {
      title: t("adminSettings.sso.role"),
      dataIndex: "role",
      key: "role",
      render: (text: string) => <Text strong>{defaultRoleDisplayNames[text]}</Text>,
    },
    {
      title: t("adminSettings.sso.mappedGroups"),
      dataIndex: "groups",
      key: "groups",
      render: (groups: string[]) => (
        <>
          {groups.length > 0 ? (
            ...
            <Text className="text-gray-400 italic">{t("adminSettings.sso.noGroupsMapped")}</Text>
          )}
        </>
      ),
    },
  ...
  return (
    <Card>
      <div className="flex items-center gap-3">
        ...
        <Title level={3}>{t("adminSettings.sso.roleMappings")}</Title>
      </div>
      <div className="space-y-8">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Title level={5}>{t("adminSettings.sso.groupClaim")}</Title>
            <div>
              <Text code>{roleMappings.group_claim}</Text>
            </div>
          </div>
          <div>
            <Title level={5}>{t("adminSettings.sso.defaultRole")}</Title>
            <div>
