...
const EditSSOSettingsModal: React.FC<EditSSOSettingsModalProps> = ({ isVisible, onCancel, onSuccess }) => {
  ...
  const handleFormSubmit = async (formValues: Record<string, any>) => {
    try {
      ...
      await mutateAsync(payload, {
        onSuccess: () => {
          NotificationsManager.success(t("adminSettings.sso.notifications.updated"));
          onSuccess();
        },
        onError: (error) => {
          NotificationsManager.fromBackend(
            t("adminSettings.sso.notifications.saveFailed", { error: String(parseErrorMessage(error)) }),
          );
        },
      });
    } catch (error) {
      ...
      NotificationsManager.fromBackend(
        t("adminSettings.sso.notifications.processFailed", { error: String(parseErrorMessage(error)) }),
      );
    }
  };
  ...
  return (
    <Modal
      title={t("adminSettings.sso.edit")}
      open={isVisible}
      width={800}
      footer={
        <Space>
