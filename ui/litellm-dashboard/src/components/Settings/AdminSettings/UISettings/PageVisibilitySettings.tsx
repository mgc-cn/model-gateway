  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      <Space direction="vertical" size={4}>
        <Space align="center">
          <Typography.Text strong>{t("adminSettings.ui.pageVisibility.title")}</Typography.Text>
          {!isPageVisibilitySet && (
            <Tag color="default" style={{ marginLeft: "8px" }}>
              {t("adminSettings.ui.pageVisibility.notSet")}
            </Tag>
          )}
          {isPageVisibilitySet && (
            <Tag color="blue" style={{ marginLeft: "8px" }}>
              {t("adminSettings.ui.pageVisibility.selected", { count: selectedPages.length })}
            </Tag>
          )}
        </Space>
        {enabledPagesPropertyDescription && (
        ...
        <Typography.Text type="secondary" style={{ fontSize: "12px", fontStyle: "italic" }}>
          {t("adminSettings.ui.pageVisibility.defaultHelp")}
        </Typography.Text>
        <Typography.Text type="secondary" style={{ fontSize: "12px", color: "#8b5cf6" }}>
          {t("adminSettings.ui.pageVisibility.note")}
        </Typography.Text>
      </Space>
      ...
      <Collapse
        items={[
          {
            key: "page-visibility",
            label: t("adminSettings.ui.pageVisibility.configure"),
            children: (
              <Space direction="vertical" size="middle" style={{ width: "100%" }}>
                            ...
                            <div key={page.page} style={{ marginBottom: "4px" }}>
                              <Checkbox value={page.page}>
                                <Space direction="vertical" size={0}>
                                  <Typography.Text>
                                    {t(`navigation.items.${page.page}`, { defaultValue: page.label })}
                                  </Typography.Text>
                ...
                <Space>
                  <Button type="primary" onClick={handleSavePageVisibility} loading={isUpdating} disabled={isUpdating}>
                    {t("adminSettings.ui.pageVisibility.save")}
                  </Button>
                  {isPageVisibilitySet && (
                    <Button onClick={handleResetToDefault} loading={isUpdating} disabled={isUpdating}>
                      {t("adminSettings.ui.pageVisibility.reset")}
                    </Button>
                  )}
                </Space>
              </Space>
